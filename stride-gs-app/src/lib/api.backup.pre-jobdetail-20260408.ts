...
export const API_TIMEOUT_ERROR = 'Request timed out after 90s — check the sheet to verify if the change was applied before retrying.';
...
export async function apiPost<T>(
  ...
  // 90-second timeout watchdog
  const timeoutCtrl = new AbortController();
  const timeoutHandle = setTimeout(() => timeoutCtrl.abort(), API_POST_TIMEOUT_MS);
...
export function postBulkSyncToSupabase(clientSheetId?: string) {
  return apiPost<BulkSyncResult>('bulkSyncToSupabase', clientSheetId ? { clientSheetId } : {});
}
