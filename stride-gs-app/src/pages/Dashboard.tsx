...
function TasksTab({ tasks, onNavigate, indicators }: { tasks: SummaryTask[]; onNavigate: (task: SummaryTask) => void; indicators?: { inspOpenItems: Set<string>; inspDoneItems: Set<string>; asmOpenItems: Set<string>; asmDoneItems: Set<string>; repairOpenItems: Set<string>; repairDoneItems: Set<string> } }) {
  ...
  const columns = useMemo(() => [
    ...
    colT.accessor('itemId', { id: 'taskItem', header: 'Item', size: 110, cell: i => { const id = i.getValue(); return <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}><span style={{ fontSize: 12, color: theme.colors.textSecondary }}>{id || '—'}</span>{id && indicators && <ItemIdBadges itemId={id} inspOpenItems={indicators.inspOpenItems} inspDoneItems={indicators.inspDoneItems} asmOpenItems={indicators.asmOpenItems} asmDoneItems={indicators.asmDoneItems} repairOpenItems={indicators.repairOpenItems} repairDoneItems={indicators.repairDoneItems} />}</div>; } }),
...
function RepairsTab({ repairs, onNavigate, userRole, indicators }: { repairs: SummaryRepair[]; onNavigate: (repair: SummaryRepair) => void; userRole?: string; indicators?: { inspOpenItems: Set<string>; inspDoneItems: Set<string>; asmOpenItems: Set<string>; asmDoneItems: Set<string>; repairOpenItems: Set<string>; repairDoneItems: Set<string> } }) {
