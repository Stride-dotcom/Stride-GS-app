...
export function Tasks() {
  ...
  // Session 77: ONE-TIME init via a ref so subsequent user changes
  ...
  // Columns menu (ColumnManagerMenu) is a portal and closes itself on outside
  // click, so the old menuRef outside-click effect is gone (it would have
  // mis-fired on clicks inside the portaled menu).
  ...
  return (
    <div style={{ background: '#F5F2EE', margin: isMobile ? '-12px -8px' : '-28px -32px', padding: isMobile ? '8px' : '28px 32px', minHeight: '100%', overflowX: isMobile ? 'clip' : undefined, maxWidth: isMobile ? '100vw' : undefined }}>
      {!isMobile && (
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 20, fontWeight: 700, letterSpacing: '2px', color: '#1C1C1C' }}>STRIDE LOGISTICS · TASKS</div>
        </div>
      )}
      ...
      {isMobile ? (
        /* Mobile: single compact status dropdown instead of a wrapping pill row
           (which ate 2-3 lines). Single-select replace (clear → toggle one). */
      ...
      <div style={mobileChipsRow(isMobile)}>
        ...
        <div style={{ flex: 1 }} />
        <span style={{ fontSize: 12, color: theme.colors.textMuted, alignSelf: 'center' }}>Showing <strong>{table.getRowModel().rows.length}</strong> of <strong>{data.length}</strong> tasks</span>
        {(sf.length > 0 || af || globalFilter || sorting.length > 0) && (
      ...
      <div style={{ border: `1px solid ${theme.colors.border}`, borderRadius: isMobile ? 8 : 12, overflow: 'hidden', background: '#fff' }}>
        <div ref={containerRef} style={{ overflowY: 'auto', overflowX: 'auto', maxHeight: isMobile ? 'calc(100dvh - 200px)' : 'calc(100dvh - 280px)', minHeight: isMobile ? 200 : undefined, WebkitOverflowScrolling: 'touch' }}>
          <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, tableLayout: 'fixed', minWidth: isMobile ? 700 : table.getTotalSize() }}>
