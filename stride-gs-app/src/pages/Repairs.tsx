...
export function Repairs() {
  ...
  // RepairPage fetches its own data from Supabase by ID.
  useEffect(() => {
    if (location.search) {
      const params = new URLSearchParams(location.search);
      const openId = params.get('open');
      if (openId) {
        navigate(`/repairs/${openId}`, { replace: true });
      }
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
    ...
    <div style={{ background: '#F5F2EE', margin: isMobile ? '-12px -8px' : '-28px -32px', padding: isMobile ? '8px' : '28px 32px', minHeight: '100%', overflowX: isMobile ? 'clip' : undefined, maxWidth: isMobile ? '100vw' : undefined }}>
      {!isMobile && (
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 20, fontWeight: 700, letterSpacing: '2px', color: '#1C1C1C' }}>STRIDE LOGISTICS · REPAIRS</div>
        </div>
      )}
      ...
      <div style={mobileChipsRow(isMobile)}>
        ...
        <div style={{ flex: 1 }} />
        <span style={{ fontSize: 12, color: theme.colors.textMuted, alignSelf: 'center' }}>Showing <strong>{table.getRowModel().rows.length}</strong> of <strong>{data.length}</strong> repairs</span>
        {(sf.length > 0 || globalFilter || sorting.length > 0) && (
