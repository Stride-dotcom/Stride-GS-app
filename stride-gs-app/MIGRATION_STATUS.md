...
> Last updated: 2026-05-09 (P1.4 reverse writethrough endpoint shipped — GAS-side `handleWriteThroughReverse_` + `REVERSE_WRITETHROUGH_TABLES_` registry of stubs, SB-side `_shared/reverse-writethrough.ts` helper. Framework only; per-table writers ship in their corresponding P2/P3/P4 PRs. StrideAPI v38.200.0.).
...
- [ ] **`GAS_API_URL` + `GAS_API_TOKEN` Edge Function secrets** — required before P2 starts so the SB-side `reverseWritethrough()` helper can call the GAS-side endpoint. Set in Supabase dashboard → Project Settings → Edge Functions → Secrets:
  - `GAS_API_URL` = the StrideAPI Web App URL (same one the React app uses)
  - `GAS_API_TOKEN` = the value of `API_TOKEN` script property in the StrideAPI Apps Script project
  Without these, any P2+ SB-primary handler that calls `reverseWritethrough()` throws `GAS_API_URL env var not set` and either crashes (strict mode) or no-ops with a `gs_sync_events` row (best-effort mode).
