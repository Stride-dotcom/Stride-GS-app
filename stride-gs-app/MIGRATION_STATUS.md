...
> Last updated: 2026-06-01 — **[MIGRATION-P1.9 / parity hardening] `receiveShipment` + `createTask` audit-shape coercion now mirrors GAS's `||` byte-for-byte; the rolling 7-day mismatch count will heal as legacy pre-PR-564-deploy rows age out.** Branch `fix/migration/recvship-createtask-parity`. PR #564 (merged + deployed 2026-05-29 02:55 PT — `deploy: merge PRs 563+564: writethrough field coverage + shadow EF updates`) added the correct `{itemCount, carrier}` / `{summary, svcCodes}` audit-shape overrides on both shadow EFs AND the registry. Live verification today (2026-06-01) — curl against `receive-shipment-shadow` and `create-task-shadow` returns the GAS-matching shape on every payload tested; the deployed React bundle (`index-BUgfzkA0.js`) contains both registry overrides. So **no current logic divergence** between EF and registry. The 30/54 receiveShipment match rate the dashboard shows today is the **expected mid-transition state**: a rolling 7-day window catches 4 days of pre-deploy mismatches (5/25–5/28) + 3 days of post-deploy matches (5/29–6/01). At ~6–8 calls/day, that math produces the observed 24 mismatch + 30 match split. By 2026-06-05 the 7-day window slides past the deploy and `mismatch_count_7d` returns to 0. Same pattern explains `createTask`'s 5/5. **Defensive code tightening shipped this session**: switched both `receive-shipment-shadow` and `shadowRegistry.ts → completeShipment` from `String(p.carrier ?? '')` to `(p.carrier as ...) || ''`, mirroring GAS at StrideAPI.gs:9598 byte-for-byte. The previous form diverged from GAS only for non-string carrier values (0, false, NaN, number) — never observed in production but a single webhook/retry/integration path passing a non-string would have hash_diff'd silently. Same `||` semantics now in EF, registry, and GAS for both `items` and `carrier`. **No new deploy required** for the parity recovery itself (it heals on the 7-day window). The shadow-EF redeploy + React deploy for the defensive coercion change are operator-pending per `feedback_supabase_deploy_token.md`. Per-function table updated below; `receiveShipment` + `createTask` parity-check rows now reflect real volume.
...
| `createTask` | gas | 10 (5/5 in 7d to 6/01) | recovering — 50% on rolling 7d, **healing**: post-deploy (5/29+) calls all match per curl; pre-deploy mismatches age out by 6/05 | 0 | Justin Demo (pending operator flag-flip — MIG-016) | **handler_drafted** (`batch-create-tasks-sb` real handler built, awaiting deploy; audit-shape fixed PR #564 + tightened to `\|\|` semantics PR `recvship-createtask-parity`) | 2026-06-01 |
...
| `receiveShipment` | gas | 54 (30/24 in 7d to 6/01) | recovering — 55% on rolling 7d, **healing**: post-deploy (5/29+) calls all match per curl; pre-deploy mismatches age out by 6/05 | 0 | n/a | **handler_drafted** (`receive-shipment-shadow` deployed 5/19; audit-shape fixed PR #564 + tightened to `\|\|` semantics PR `recvship-createtask-parity`) | 2026-06-01 |
