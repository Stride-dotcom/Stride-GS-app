...
> Last updated: 2026-05-11 — 3 corrections to the function inventory per Justin's review: (1) **Task Board project is DECOMMISSIONED** (replaced by React app when it was created — no operators use it). (2) **Master Price List email-template functions are dead code** — templates moved to Supabase. (3) **Shipment counter `getNextShipmentId` is still on the racy Master-RPC counter** (verified by code inspection; Justin's recollection that it had moved doesn't match the code — needs the same v38.182-style SEQUENCE migration). Inventory + open questions updated accordingly.
...
- [ ] **`getNextShipmentId` racy counter still in use** — confirmed 2026-05-11 via code inspection: `api_nextShipmentNo_` (StrideAPI.gs:14803) still hits Master-RPC with `action: "getNextShipmentId"`, and no `next_shipment_no()` SEQUENCE exists in Supabase. The v38.182 atomic SEQUENCE migration was invoice-only. Justin's recollection ("I think it may have been moved") doesn't match the code — shipment numbering is still on the racy counter, with the same dup-number-race risk class as the old invoice counter. **Action:** add `getNextShipmentId` to the per-function migration table (likely P5 alongside `receiveShipment`, or its own mini-phase). Mirror the v38.182 pattern: a `public.shipment_no_seq` Postgres SEQUENCE + `public.next_shipment_no()` SQL function + `api_nextShipmentNo_` rewrite to call it. The `doPost` route in `Master Price list script.txt` can't be fully retired until this migrates.
