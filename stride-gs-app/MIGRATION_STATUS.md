...
> Last updated: 2026-05-16 — **P4a billing-core shadows: createInvoice / voidInvoice / generateUnbilledReport** (branch `feat/migration/p4a-invoice-shadow`, PR pending). Three SHADOW/parity Edge Functions (`create-invoice-sb`, `void-invoice-sb`, `generate-unbilled-report-sb`) port the COMPUTE half of the GAS handlers — zero writes, zero PDF/email, no invoice-number consumption (atomic-counter landmine respected). Migration `20260516120000_p4a_invoice_shadow_feature_flags.sql` turns parity ON for createInvoice/voidInvoice (guarded `WHERE active_backend='gas'`) + seeds net-new `generateUnbilledReport`. **active_backend stays 'gas' fleet-wide.** Per MIG-007 layer-1 (per-call live shadow — the caller diffs the returned `parity` object); layer-2 historical replay (SHADOW_REGISTRY/replay-shadow) intentionally NOT wired — invoice creation has no fixed audit-changes shape, parity is data-derived (rationale shape mirrors MIG-013's replay deferral). **Doc-drift correction:** the phase/per-function tables below said `P4a — not_started` but git shows `completeTask` (#447) + `completeRepair` (#419) P4a already shipped; corrected P4a→in_progress + my 3 rows. completeTask/completeRepair per-function rows were NOT updated by their sessions and still read stale — their owners should reconcile (not rewritten here to avoid clobbering another session's intent). **Env note (matches #443/#444 precedent):** no Supabase MCP / `SUPABASE_ACCESS_TOKEN` and no registered `code-reviewer` subagent in this harness — code review ran the `.claude/agents/code-reviewer.md` definition verbatim via an Opus general agent (one Critical `esc()` parity bug found + fixed); Edge Functions + migration are COMMITTED BUT NOT DEPLOYED/APPLIED — operator commands handed to Justin in BUILD_STATUS "Pending User Actions". **Known parity caveats:** (a) `create-invoice-sb` PAYMENT_TERMS reads `public.clients.payment_terms` (GAS reads per-tenant sheet Settings) — sunsets when client Settings move to SB; (b) `generate-unbilled-report-sb` date-normalization fast-paths the 3 stored formats, exotic/locale date strings could differ (differ surfaces it).
>
...
|---|---|---|---|---|
| `/c/dev/stride-mig-invoice` | `feat/migration/p4a-invoice-shadow` | P4a | createInvoice / voidInvoice / generateUnbilledReport shadows (parity-only) | 2026-05-16 (remove at merge) |
...
| **P4a — billing core** | in_progress | `completeTask`, `completeRepair`, `processWcRelease`, `commitStorageCharges`, `createInvoice`, `voidInvoice`, `reissueInvoice` | Per-tenant + SB mirror + `invoice_tracking`. completeTask (#447) + completeRepair (#419) SB-primary shipped; createInvoice/voidInvoice/generateUnbilledReport shadows shipped 2026-05-16 (PR pending). |
...
| `createInvoice` | gas | 0 | n/a | 0 | n/a | **handler_drafted (shadow)** | 2026-05-16 (PR pending — `create-invoice-sb` shadow/parity, compute-only: line items, totals, PDF tokens, QB customer name, sidemark assertion. No writes/PDF/email/invoice-no. parity-on, fleet GAS. MIG-007 layer-1 only; layer-2 deferred.) |
| `voidInvoice` | gas | 0 | n/a | 0 | n/a | **handler_drafted (shadow)** | 2026-05-16 (PR pending — `void-invoice-sb` shadow/parity: which public.billing rows would flip Invoiced→Void. No writes, CB symmetry GAS-side only. parity-on, fleet GAS.) |
| `reissueInvoice` | gas | 0 | n/a | 0 | n/a | not_started | — |
| `generateUnbilledReport` | gas | 0 | n/a | 0 | n/a | **handler_drafted (shadow)** | 2026-05-16 (PR pending — `generate-unbilled-report-sb` shadow/parity: cross-tenant Unbilled row-set from public.billing. Read-only, no CB Unbilled_Report sheet write. Net-new feature_flags row, fleet GAS.) |
