...
> Last updated: 2026-06-09 — **[feat/orders/sb-order-numbering, PR #695] Clean SB-generated order numbering shipped (Justin Demo canary).** Net-new feature (not a handler migration). New orders for the Justin Demo tenant get clean client-scoped sequential ids — repairs `PREFIX-RPR-N`, will calls `PREFIX-WC-N`, tasks `PREFIX-TSK-N`, delivery `PREFIX-N` (zero-padding dropped) — replacing GAS timestamp ids. Gated via a new `orderNumbering` feature_flags row (UI/behavior gate, NOT apiRouter routing; MIG-010 per-tenant scope, `tenant_scope=[justinDemo]`). Migration `20260609120000_sb_order_numbering.sql` adds `order_sequences` (per-tenant per-type counter; NOT a parity mirror — same as `invoice_no_seq`/`shipment_no_seq`) + `next_order_number(tenant,type)` atomic increment + `order_client_prefix` (mirrors `buildOrderNumberBase`) + `order_numbering_enabled` (matches `resolveFlagBackend`) + `next_order_id` composer (NULL→legacy fallback). The **fleet-wide** `create_repair_quote_request` RPC was gated in-place (clean id when enabled, else `next_repair_id` — zero change for production tenants). EFs `create-will-call-sb` v6 + `batch-create-tasks-sb` v5 redeployed; `CreateDeliveryOrderModal` strips lpad; `LinkifiedText` recognizes the new `PREFIX-TOKEN-N` shape. Reverse-writethrough to the sheet is automatic (existing repairs/will_calls/tasks writers push the PK column). Existing rows keep old ids (no renumbering). Revert = flip the flag to `gas`.

> Last updated: 2026-06-08 — **[docs/audit-refresh] Doc-audit pass: the per-function table below is SUPERSEDED by the live scorecard.** Since the 2026-05-19 snapshot the migration crossed its inflection point: **every GAS write-handler now has a deployed, flag-routed `-sb` Edge Function, and the Justin Demo Account tenant runs 100% on Supabase.** Headline state as of today (see **## Migration Scorecard (2026-06-08)** immediately below for the full breakdown): **9 handlers fleet-wide on Supabase, 54 handlers canary-on-Supabase (Justin Demo only — production tenants stay on GAS via MIG-010 opposite-backend), 4 decoy routing rows (`updateTaskNotes`/`updateTaskDueDate`/`updateTaskPriority`/`updateTaskCustomPrice` all resolve through the `updateTask` flag → `update-task-sb`), and 0 real GAS-only handlers left** (no write action lacks an SB equivalent). All `-sb` and grouped EFs are built + deployed (110 functions in `supabase/functions/`). **QBO OAuth secrets have been mirrored to Supabase EF secrets** — the `qboCreateInvoice` / `qboReconcile` flips are no longer credential-blocked (the long-standing CONFIG_ERROR gate is cleared; production fleet-flip is now an operator decision, not a prerequisite). The per-function state-machine table further down is retained as a historical 5/19 snapshot; **for current backend/scope, read the scorecard + the Settings → Migration tab (live `feature_flags`), which is authoritative.** — Earlier 2026-06-05 — **[feat/billing/cod-storage] New COD Storage feature (NOT a migration handler) — Supabase-first, Justin-Demo-gated.** Adds `codStorageBilling` to `feature_flags` (active_backend=supabase, tenant_scope=[justinDemo]) used purely as a UI gate (no routing/shadow — net-new feature, nothing to shadow). Schema touches the parity mirror set: `inventory.cod_storage`+`cod_storage_start_date` and `clients.end_customer_pays_storage` each shipped with matching `parity_dryrun.*` ALTERs (schema-sync convention honored; `check_drift()` clean for these). `dt_orders` (non-mirror) got COD columns. New SECURITY DEFINER RPCs `set_cod_storage` + `mark_cod_storage_collected`; `_compute_storage_charges` gained a COD cap (verbatim copy + one LEAST). All migrations applied to prod; `dt-push-order` redeployed v46 (deploy-version 47, verify_jwt=false). **Pre-existing drift noted (not mine): `parity_dryrun.invoice_tracking` missing 6 qbo_*/pdf_path columns from the 2026-06-04 reconcile work — flagged for a follow-up sync.** — Earlier 2026-06-05 — **[MIGRATION-P6 / payments] Stax "test invoice" is now 100% Supabase — first Stax write fully off GAS.** Branch `feat/payments/test-stax-invoice-sb`. New native EF `create-test-stax-invoice` (NOT a `-sb` proxy, NOT a shadow): MIG-017 admin/staff gate → resolve/create Stax customer id (caller-supplied → `public.clients` lookup → `POST /customer` + persist back) → insert `public.stax_invoices` (`is_test=true`, PENDING, row shape == legacy GAS `handleCreateTestInvoice_`) → optional immediate `POST /invoice` to Stax (`pushToStax`, default true) stamping `stax_id`+CREATED. Fail-closed on push failure (PENDING row survives + `success:false`, the #632 guard) so the batch `create-stax-invoices-sb` can still push it. **Routing is deliberately bypassed:** React `Payments.tsx` invokes the EF via `supabase.functions.invoke` directly (NOT apiPost/`GAS_TO_SB_MAP`/feature-flag) — `createTestInvoice` removed from `GROUPED_STAX_ACTIONS` + `stax-actions-sb`. This is the **Stax-migration proving ground**: validating the Stax API integration works through SB de-risks migrating `createStaxInvoices`/`runStaxCharges` off GAS. `STAX_API_KEY`+`SUPABASE_ANON_KEY` already in EF secrets; deployed `verify_jwt=true`. No feature flag, no schema change, no billing-counter touch. Legacy GAS `handleCreateTestInvoice_` left intact (unused; P7 cleanup). **Follow-up same day (`fix/payments/test-stax-chargeable`):** the SB-only row was invisible on the Payments hard-refresh (noCache → GAS sheet read) AND uncharageable — the still-GAS charge path (`handleChargeSingleInvoice_`/`handleRunStaxCharges_`) finds invoices by reading the Stax sheet, which never had the EF's row. Added **MIG-002 reverse-writethrough for `stax_invoices`** (implements that long-standing per-table stub): new GAS `staxSheetUpsert` action (`handleStaxSheetUpsert_`, StrideAPI v38.263.0 — idempotent, header-resolved, token-gated like `writeThroughReverse`, sheet-only no SB re-mirror), the EF best-effort `gasProxy('staxSheetUpsert')` after its SB write/push, and a Payments `loadData` merge of SB-only rows on the GAS path. So the EF stays SB-primary (authoritative SB write + the Stax `POST /invoice` — the proving-ground value) while the sheet mirror keeps the not-yet-migrated charge/refresh paths working; the mirror retires when `runStaxCharges` migrates to SB. — Earlier 2026-06-04 — **[MIGRATION-P6 / payments] `qboReconcileInvoices` migrated to a native SB Edge Function — `qbo-reconcile-payments` (NO GAS).** Branch `feat/payments/qbo-reconcile-ef`. v38.242.0 shipped the QBO payment-status reconcile as a GAS handler (`handleQboReconcileInvoices_`) + the `invoice_tracking` payment columns (`20260528160000`) + the React "Reconcile with QBO" button. This PR completes the migration of that action: a native Deno EF queries QBO for every pushed invoice (Phase 1 per-id `GET /invoice/{id}` for rows with `qbo_invoice_id`; Phase 2 bulk Query API `SELECT … FROM Invoice WHERE TxnDate >= … MAXRESULTS 1000 STARTPOSITION n` for historical/pre-fix rows, matched by DocNumber→invoice_no then `Stride INV# X` in PrivateNote), writes `qbo_balance`/`qbo_paid`/`qbo_doc_number`/`qbo_invoice_id`/`qbo_last_verified_at` back onto `invoice_tracking`, and logs "pushed-per-Stride but missing in QBO" rows to `billing_activity_log` (action=`qbo_push_failed`). OAuth refresh + token handling cloned from `qbo-create-invoice-sb`. **Auth:** admin-user JWT (React button, role=`admin`) OR service-role bearer (the daily pg_cron sweep bypasses the user-role check). **Routing:** new `GAS_TO_SB_MAP.qboReconcileInvoices → { ef: 'qbo-reconcile-payments', flagKey: 'qboReconcile' }`; migration `20260604120000_qbo_reconcile_payments_route.sql` seeds the flag at `active_backend='gas'`. **🚩 FLIP HELD — QBO secrets not in Supabase.** The QBO OAuth creds (`QBO_CLIENT_ID/SECRET/REFRESH_TOKEN/REALM_ID`) live ONLY in GAS Script Properties (set at runtime by the OAuth callback), not in Supabase EF secrets — confirmed by invoking the deployed EF (returns `CONFIG_ERROR` with the 4 missing names) and by `GET /v1/projects/.../secrets` (QBO_* absent). Same gap applies to the sibling `qbo-create-invoice-sb`, which has never been flag-flipped either. So the EF is deployed + verified-functional (auth path runs; service-role cron path validated) but cannot reach QBO until an operator mirrors those 4 secrets. Flipping React to the EF before then would `CONFIG_ERROR` the admin button in prod while the GAS handler still works — so the flag is held on `gas` (verification-column-only writes mean no MIG-016 writethrough gate; the gate here is purely the credential mirror). **Go-live (operator, 2 steps): (1) `npx supabase secrets set QBO_CLIENT_ID=… QBO_CLIENT_SECRET=… QBO_REFRESH_TOKEN=… QBO_REALM_ID=… --project-ref uqplppugeickmamycpuz` (copy from GAS Script Properties or re-run QBO OAuth); (2) `UPDATE feature_flags SET active_backend='supabase' WHERE function_key='qboReconcile'` + schedule the cron.** **UI:** the existing Invoices-header button is now admin-only, relabeled "Sync Payment Status", and sweeps ALL pushed-but-unverified invoices (empty scope, server-capped 500/run — covers the ~229-invoice backlog in one pass); the realtime `invoice_tracking` subscription refreshes the Paid/Unpaid badges automatically. **Cron:** `20260604130000_qbo_reconcile_cron.sql` installs pg_cron + pg_net; the env-specific `cron.schedule('qbo-reconcile-payments-daily', '17 9 * * *', …)` is applied out-of-band (embeds the service-role JWT — same pattern as `dt-sync-statuses`). Revert: flip `qboReconcile` flag → `gas` (GAS handler still present).
...
## Migration Scorecard (2026-06-08)

**Authoritative headline state. Supersedes the 5/19 per-function table near the bottom of this file** (kept as a historical snapshot). For live per-row backend/scope, read the **Settings → Migration tab** (`feature_flags`).

| Bucket | Count | Meaning |
|---|---|---|
| **Fleet-primary (Supabase, all tenants)** | **9** | Graduated — every tenant routes to the SB handler. |
| **Canary (Supabase, Justin Demo only)** | **54** | SB-primary for the Justin Demo tenant; all production tenants stay on GAS (MIG-010 opposite-backend). 42 individual handlers + 12 grouped-EF clusters. |
| **Decoy routing rows** | **4** | `updateTaskNotes` / `updateTaskDueDate` / `updateTaskPriority` / `updateTaskCustomPrice` — distinct React actions that all resolve through the single `updateTask` flag → `update-task-sb` (the legacy bare `updateTask` action name no React caller ever sent, so these aliases are what actually flip; see `apiRouter.ts`). |
| **Real GAS-only handlers remaining** | **0** | Every GAS write action now has a deployed, flag-routed `-sb` (or grouped) Edge Function. |

**The 9 fleet-primary handlers** (flipped `active_backend='supabase'`, `tenant_scope=NULL`):
`cancelRepair`, `startRepair`, `sendRepairQuote`, `respondToRepairQuote`, `requestRepairQuote`, `completeRepair` (the 6-handler repair cluster, fleet-flipped 2026-05-14 per MIG-013 — low-volume, Path-C), plus `updateShipment`, `generateStorageCharges`, and `updateClient` (fleet-flipped via `20260528170000_update_client_sb_route.sql`).

**The 54 canary handlers** (SB-primary on Justin Demo via `tenant_scope=[justinDemoId]`, GAS for everyone else):
- **42 individual:** `updateItem`, `updateTask`, `updateRepair`, `startTask`, `createTask`, `createWillCall`, `releaseItems`, `releaseWillCall`, `completeTask`, `processWcRelease`, `commitStorageCharges`, `createInvoice`, `voidInvoice`, `reissueInvoice`, `transferItems`, `receiveShipment`, `onboardClient`, `createLocation`, `qboCreateInvoice`, `qboReconcile`, `createStaxInvoices`, `runStaxCharges`, `importIIF`, `sendShipmentEmail`, `sendWillCallEmails`, `sendTaskCompleteEmail`, `generateUnbilledReport`, `batchUpdateItemLocations`, `cancelTask`, `reopenTask`, `batchCancelTasks`, `batchCancelRepairs`, `updateWillCall`, `cancelWillCall`, `addItemsToWillCall`, `removeItemsFromWillCall`, `syncClientBilling`, `addManualCharge`, `voidManualCharge`, `updateBillingRow`, `voidUnbilledRows`, `codStorageBilling` (net-new UI-gate feature, not a GAS migration).
- **12 grouped-EF clusters** (one EF dispatches many actions via `body.action`, proxying to GAS for un-migrated members — see `feedback_grouped_sb_efs`): `marketingActions`, `claimActions`, `staxActions`, `qbActions`, `repairExtras`, `wcExtras`, `taskBatchOps`, `billingExtras`, `locationActions`, `adminUsers`, `emailTemplates`, `clientSetup`.

**What changed since 5/19:** the migration moved from "100% shadow coverage, 0 flips" to "every handler built + deployed, demo tenant fully cut over." The remaining work is **production fleet-expansion** — promoting canary handlers from Justin-Demo-only to fleet-wide after MIG-007 layer-2/layer-3 verification per handler. The QBO-secrets blocker (which held `qboCreateInvoice`/`qboReconcile`) is **resolved** — secrets are mirrored to SB.

**Routing reference:** the action → EF → flag map is `GAS_TO_SB_MAP` in `src/lib/apiRouter.ts`; grouped clusters are the `GROUPED_*_ACTIONS` constants in the same file. The Stax `createTestInvoice` path is deliberately NOT in the grouped map — it invokes `create-test-stax-invoice` directly (100% SB, no GAS, no flag).
...
(Empty rows after merge. Add yourself at session start; remove at session end. **Repair P3/P4a cluster COMPLETE** — all 6 handlers shipped, deployed, and **graduated fleet-wide** (`cancelRepair` #405, `startRepair` #406, `sendRepairQuote` #407, `respondToRepairQuote` #408, `requestRepairQuote` #418, `completeRepair` #419; re-quote flow #420). No migration work currently in flight; the active frontier is **per-handler production fleet-expansion** of the 54 Justin-Demo canary handlers — see the scorecard.)

---

## Phase status

> **2026-06-08:** Every phase's handlers are now **built + deployed + canary-active on Justin Demo** (see the scorecard above). "in_progress" below now means "handlers shipped, canary-live on the demo tenant, awaiting production fleet-expansion" — not "still being written." The repair cluster (P3/P4a) is the only set that has graduated **fleet-wide**. P4b (CB retirement) and P7 (decommission) remain genuinely not-started.
...
| P2 — simple writes | **canary-live (Justin Demo)** | `updateItem`, `updateTask` (+4 decoy aliases), `updateRepair`, `updateShipment` | All `-sb` handlers built + deployed + routed via `GAS_TO_SB_MAP`. **`updateShipment` graduated fleet-wide;** `updateItem`/`updateTask`/`updateRepair` canary on Justin Demo. |
| P3 — status changes | **canary-live (Justin Demo)** | `startTask`, `startRepair`, `createTask`, `createWillCall`, `releaseItems`, `transferItems`, `releaseWillCall`, status-only emails (shipment/WC/repair-quote/task-complete) | Repair-cluster status handlers (`startRepair`, `sendRepairQuote`, `respondToRepairQuote`, `requestRepairQuote`, `cancelRepair`) graduated **fleet-wide**; the rest canary on Justin Demo. |
| P4a — billing core | **canary-live (Justin Demo)** | `completeTask`, `completeRepair`, `processWcRelease`, `commitStorageCharges`, `createInvoice`, `voidInvoice`, `reissueInvoice` | All `-sb` built + deployed. **`completeRepair` + `generateStorageCharges` graduated fleet-wide;** `completeTask`/`processWcRelease`/`commitStorageCharges`/`createInvoice`/`voidInvoice`/`reissueInvoice` canary on Justin Demo. ⚠️ `commitStorageCharges` cutover note still applies (the `commit-storage-charges-sb` EF proxies the Create-Invoice COMMIT to GAS `handleCommitStorageRows_` so per-item `storage_billing_items` writes are preserved — see PR #673 in BUILD_STATUS). |
| P4b — CB retirement | **not_started** | CB `Consolidated_Ledger` retire + QBO direct push (replacing IIF) | Prereq met: `qboCreateInvoice` EF shipped + QBO secrets now mirrored to SB. CB sheet retirement itself not begun. |
| P5 — complex flows | **canary-live (Justin Demo)** | `receiveShipment`, `transferItems`, `onboardClient`, `updateClient`, `createLocation` | `complete-shipment-sb`, `transfer-items-sb`, `onboard-client-sb` (HYBRID — GAS retains Drive/Sheets provisioning), `update-client-sb`, `create-location-sb` deployed. **`updateClient` graduated fleet-wide.** |
| P6 — payments | **canary-live (Justin Demo)** | `qboCreateInvoice`, `qboReconcile`, `createStaxInvoices`, `runStaxCharges`, `importIIF` | Real-money handlers built + deployed; admin/staff role gate per MIG-017. **QBO OAuth secrets now mirrored to SB EF secrets — the CONFIG_ERROR blocker is cleared.** `stax_invoices` is fleet-wide by design (no tenant_id) — the role gate is the security boundary. Stax `createTestInvoice` is already 100% SB (direct EF, no flag). Production fleet-flip is now an operator decision. |
| P7 — decommission | **partial** | GAS write-handler stubs, per-client GAS v5.0.0 freeze, time-driven trigger migration to pg_cron; the 12 grouped-EF clusters | Grouped EFs (`marketing-actions-sb` … `client-setup-sb`) are built + deployed + canary-routed, proxying un-migrated actions to GAS. GAS handlers NOT yet stubbed/frozen — they remain the production path for non-demo tenants. |
...
> ⚠️ **HISTORICAL SNAPSHOT (2026-05-19) — SUPERSEDED by the [Migration Scorecard (2026-06-08)](#migration-scorecard-2026-06-08) near the top of this file.** Every row below reads `gas` / `handler_drafted`; that was true on 5/19 but is no longer current. As of 2026-06-08, 9 handlers are fleet-primary, 54 are canary-active on Justin Demo, and 0 remain GAS-only. This table is retained for the parity-check counts and per-function notes (which are still accurate as of 5/19). **For current backend/scope, use the scorecard + the live Settings → Migration tab.**
...
| `commitStorageCharges` | supabase (canary) | 0 | n/a | 0 | Justin Demo (LIVE 6/9) | **CUT OVER (canary) 2026-06-09** — SB-native `commit_storage_rows` (PR #694, migration `20260609000000`) replaces GAS `handleCommitStorageRows_`; EF `commit-storage-charges-sb` v6 rewired off `gasProxy` → RPC + sheet mirror. Writes per-item `storage_billing_items` + sidemark-aware summaries — **the 2026-06-03 blocker is RESOLVED** (sbi write + `billable_days` = qty else round(amount/rate) both ported). PRECISE-REMAINDER: bills the un-billed remainder of partially-billed items (no GAS `sbiAlreadyBilled` coarse skip), recovering ~$284/mo. Double-bill blocked structurally (fence + `uq_sbi_active_item_period`). Validated live on canary (1-row commit: billing+sbi+sheet consistent). Fleet rollout pending: reverse-`op=delete` for stale sheet summaries + monitoring + flag-scope expansion. | 2026-06-09 |
| `generateStorageCharges` | supabase (canary) | 0 | n/a | 0 | Justin Demo | recompute RPC `generate_storage_charges` brought to parity 2026-06-08 (PR #678, migration `20260608238000`): now writes per-item sbi + sidemark-aware summaries (closing the same blocker). **Dormant** (no live React caller); routes via `commit-storage-charges-sb` no-`rows[]` branch under the same `commitStorageCharges` flag. | 2026-06-09 |
...
- [x] ~~**GO-LIVE for `qbo-reconcile-payments` — QBO secrets blocker**~~ — **RESOLVED 2026-06-08.** The QBO OAuth secrets (`QBO_CLIENT_ID/CLIENT_SECRET/REFRESH_TOKEN/REALM_ID`) have been **mirrored from GAS Script Properties into Supabase EF secrets**, clearing the CONFIG_ERROR gate that held both `qbo-reconcile-payments` and the sibling `qbo-create-invoice-sb`. Both are now canary-live on Justin Demo. Production fleet-flip (`UPDATE feature_flags SET active_backend='supabase', tenant_scope=NULL WHERE function_key IN ('qboReconcile','qboCreateInvoice')` + schedule the cron from `20260604130000_qbo_reconcile_cron.sql`) is now a normal operator decision, not credential-blocked.
...
- [x] ~~**Nominate a canary tenant**~~ — **RESOLVED.** The **Justin Demo Account** is the canary tenant (MIG-016). It now runs **100% on Supabase** — 54 handlers are SB-primary scoped to it, plus the 9 already-fleet-wide. The remaining work is per-handler MIG-007 layer-2/layer-3 verification before promoting each canary handler from Justin-Demo-only to fleet-wide for production tenants.
