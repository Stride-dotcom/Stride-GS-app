...
> Last updated: 2026-05-22 (round 3) — **[MIGRATION-P2-P5 / MIG-016] Round 3 adds 4 more real -sb handlers — task/repair field updates + shipment receiving + cross-tenant transfer.** PR #502 (round 1+2) merged 2026-05-22T06:20Z; React deployed live. Round 3 in new PR (this branch). New handlers: `update-task-sb` (consolidates 4 GAS task-update actions into one EF; all share `updateTask` flag), `update-repair-sb` (mirrors handleUpdateRepairNotes_'s broader field set), `complete-shipment-sb` (real port — `next_shipment_no()` SEQUENCE, `[IK:]` dedup, dup-item guard, inventory + auto-INSP/ASM tasks + receiving billing with service_catalog rate + per-client discount; canary skips Drive folders + email), `transfer-items-sb` (cross-tenant move — validate dest, dup guard, source→Transferred, dest→Active rows, void source Unbilled + recreate dest with dest's discount; canary skips open Tasks/Repairs port). GAS_TO_SB_MAP gains 9 entries (4 task variants + updateRepairNotes + completeShipment + transferItems). Per-function table updated below. Deployment is operator-pending per BUILD_STATUS.md Pending User Actions (9 EFs total across rounds 1-3).
...
| `updateTask` | gas | 26 | **100%** (26/0) | 0 | Justin Demo (pending operator flag-flip — MIG-016) | **handler_drafted** (`update-task-sb` real handler built — round 3, awaiting deploy) | 2026-05-22 |
| `updateRepair` | gas | 1 | **100%** (1/0) | 0 | Justin Demo (pending operator flag-flip — MIG-016) | **handler_drafted** (`update-repair-sb` real handler built — round 3, awaiting deploy) | 2026-05-22 |
...
| `transferItems` | gas | 5 | **100%** (5/0) | 0 | Justin Demo (pending operator flag-flip — MIG-016) | **handler_drafted** (`transfer-items-sb` real handler built — round 3, awaiting deploy; open Tasks/Repairs port to destination deferred) | 2026-05-22 |
| `receiveShipment` | gas | 0 | n/a | 0 | Justin Demo (pending operator flag-flip — MIG-016) | **handler_drafted** (`complete-shipment-sb` real handler built — round 3, awaiting deploy; Drive folders + email skipped) | 2026-05-22 |
