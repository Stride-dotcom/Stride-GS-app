...
> Last updated: 2026-05-16 — **P3/P5 operational shadows: 5 pure shadow Edge Functions shipped** (`create-will-call-shadow`, `release-will-call-shadow`→action `processWcRelease`, `create-task-shadow`→action `batchCreateTasks`, `release-items-shadow`, `transfer-items-shadow`). All Path-C pure (read-only, mirror the GAS `api_auditLog_` answer-key shape; no DB writes, no GAS HTTP). Registered in `replay-shadow` SHADOW_REGISTRY (now 11 entries). Migration `20260516120000_p3_ops_shadow_feature_flags.sql` flips `shadow_backend='supabase'`+`parity_enabled=true` for `createWillCall`/`releaseWillCall`/`createTask`/`releaseItems`/`transferItems` (active stays `gas`, MIG-007). **Deploy + `apply_migration` pending operator** — the builder env has no `SUPABASE_ACCESS_TOKEN`, so the 5 `supabase functions deploy` calls and the SQL apply are listed under "Pending user actions". Deliberately NOT built this session (no clean entity_audit_log answer key / no gas_call_log corpus / owned by another in-flight worktree): `reissueInvoice` (stateful audit, owned by stride-mig-invoice P4a), `commitStorageCharges` (no audit row — RPC billing write), `onboardClient` (no `api_auditLog_` call at all), `updateTask`/`updateRepair` (no single doPost action — multiple `updateTask*`/`updateRepair*` sub-actions), `sendShipmentEmail`/`sendTaskCompleteEmail`/`sendWillCallEmails` (moved off GAS to the `send-email` Resend Edge Function — no gas_call_log corpus to replay), `receiveShipment` (no `receiveShipment` doPost case; closest is `completeShipment` — mapping needs design). See "Open questions / blockers". **Code review also fixed a latent harness bug**: `replay-shadow` hardcoded `entity_type='inventory'` on the answer-key fetch (updateItem-MVP leftover), which had silently made the already-shipped repair P3 + `completeRepair` + `completeTask` shadows registered-but-inert (all `no_audit_row`). `ShadowEntry` now carries per-entry `entity` + `inputShapedChanges`; this PR therefore also unblocks parity scoring for the whole prior repair/task cluster once `replay-shadow` is re-deployed.
>
> Prior: 2026-05-13 (later) — **P3 repair cluster: 4-of-6 handlers shipped via Path-C** (PRs #405 + #406 + #407 + #408). `cancelRepair` (smoke-tested), `startRepair`, `sendRepairQuote` (+ Resend email), `respondToRepairQuote` (+ Resend Approved/Declined email). All deployed at `active_backend='gas'`; flip in Settings → Migration to activate (gas → supabase, `tenant_scope=NULL` for fleet-flip — Justin's plan since repairs are low-volume). Two remaining for the cluster: **`requestRepairQuote` single-item** (~30 min — wire TaskDetailPanel + ItemDetailPanel to existing `request-repair-quote-sb` with `itemIds:[oneItem]`) and **`completeRepair`** (P4a, ~4-5 hrs — new `__writeThroughReverseBilling_` writer for per-tenant Billing_Ledger + new `mirrorBillingToCb` GAS endpoint for the separate CB spreadsheet + REPAIR_COMPLETE email). Architectural call confirmed: SB needs only ONE billing table (`public.billing` with `tenant_id` column), not two — the per-tenant + consolidated split exists in GAS only because Google Sheets can't aggregate across spreadsheets. StrideAPI bumped to v38.216.0 — `__writeThroughReverseRepairs_` covers all 17 repair columns including the 8 quote_* fields. SHADOW_REGISTRY in `replay-shadow` lists 5 entries (updateItem + 4 repair P3). Critical security pattern carried through every handler: JWT signature verified via `supabase.auth.getUser(token)` against anon-keyed client (NOT just `atob` decode — the cancelRepair code review caught the forgeable-token issue and the fix propagated to all subsequent handlers).
...
|---|---|---|---|---|
| `/c/dev/stride-mig-ops` | `feat/migration/p3-ops-shadows` | P3/P5 | 5 ops shadows (createWillCall, releaseWillCall, createTask, releaseItems, transferItems) + SHADOW_REGISTRY + feature_flags migration. PR open; deploy/SQL pending operator. Remove this row after merge + operator deploy. | 2026-05-16 |
...
| `createTask` | gas | 0 | n/a | 0 | n/a | **handler_drafted** | 2026-05-16 (create-task-shadow shipped; gas action=`batchCreateTasks`; PR open; deploy + feature_flags SQL pending operator — no SUPABASE_ACCESS_TOKEN in builder env; unflipped) |
| `createWillCall` | gas | 0 | n/a | 0 | n/a | **handler_drafted** | 2026-05-16 (create-will-call-shadow shipped; PR open; deploy + flags SQL pending operator; unflipped) |
| `releaseItems` | gas | 0 | n/a | 0 | n/a | **handler_drafted** | 2026-05-16 (release-items-shadow shipped; PR open; deploy + flags SQL pending operator; unflipped) |
...
| `processWcRelease` | gas | 0 | n/a | 0 | n/a | not_started | — (P4a atomic-billing variant; NOT the shadow shipped this session — see `releaseWillCall` below) |
| `releaseWillCall` | gas | 0 | n/a | 0 | n/a | **handler_drafted** | 2026-05-16 (release-will-call-shadow shipped; gas action=`processWcRelease`; PR open; deploy + flags SQL pending operator; unflipped) |
...
| `transferItems` | gas | 0 | n/a | 0 | n/a | **handler_drafted** | 2026-05-16 (transfer-items-shadow shipped — models source-tenant `transfer` row; dest `transfer_in` row deferred per N-rows tolerance; PR open; deploy + flags SQL pending operator; unflipped) |
...
- [ ] **Remaining ops shadows need a design decision, not just a build (2026-05-16)** — the 2026-05-16 session shipped the 5 cleanly-mappable ops shadows. The other requested handlers do NOT fit the pure-audit-shadow / replay-corpus pattern and were deliberately not built blind (a wrong shadow silently poisons `mismatch_count_7d` and could green-light a bad cutover):
  - `reissueInvoice` — writes its audit row *inside* `handleReissueInvoice_` with runtime-computed `rowsReleased`/`cbRowsDeleted`/`ledgerRowIds`. Stateful; belongs to the invoice/billing cluster (in-flight worktree `feat/migration/p4a-invoice-shadow`). Coordinate there, don't double-build.
  - `commitStorageCharges` — no `api_auditLog_` call anywhere on the storage-commit path (`generateStorageCharges` / `commitStorageRows` only `api_fullClientSync_`). No entity_audit_log answer key → needs a different parity compare target (likely a `parity_dryrun.billing` state diff). Design needed.
  - `onboardClient` — the `onboardClient` doPost case writes NO `api_auditLog_` row (only `resyncClientToSupabase_`). No answer key. Design needed (compare target = `public.clients` row?).
  - `updateTask` / `updateRepair` — no single doPost action; the React app uses `updateTaskNotes`/`updateTaskCustomPrice`/`updateTaskDueDate`/`updateTaskPriority` and `updateRepairNotes`/`correctRepairResult`/etc. Each has its own audit shape. Either one shadow per sub-action or a dispatching shadow — pick before building.
  - `sendShipmentEmail` / `sendTaskCompleteEmail` / `sendWillCallEmails` — these moved off GAS to the `send-email` Resend Edge Function (per the substrate seed notes "P3 — move to send-email Edge Function"). There is no `gas_call_log` corpus to replay and no entity_audit_log answer key. Parity here means something different (did the SB path send the same email?) — needs its own verification design, not a replay shadow.
  - `receiveShipment` — no `receiveShipment` doPost case. Closest is `completeShipment` (audits `shipment`/`create` `{itemCount,carrier}`). Confirm whether the React "receive shipment" flow IS `completeShipment` before registering, and note it's P5 multi-entity (inventory + RCVG billing + auto-INSP + email) so the audit-changes answer key may be insufficient on its own.

---

## Pending user actions

- [ ] **Deploy the 5 P3/P5 ops shadow Edge Functions** (builder env has no `SUPABASE_ACCESS_TOKEN`, so this must run wherever the operator's token is set — e.g. `supabase login` first, or `SUPABASE_ACCESS_TOKEN=… `). After PR `feat/migration/p3-ops-shadows` merges to `source` and `git pull`:
  ```bash
  cd /c/dev/Stride-GS-app/stride-gs-app
  npx supabase functions deploy create-will-call-shadow  --project-ref uqplppugeickmamycpuz
  npx supabase functions deploy release-will-call-shadow --project-ref uqplppugeickmamycpuz
  npx supabase functions deploy create-task-shadow       --project-ref uqplppugeickmamycpuz
  npx supabase functions deploy release-items-shadow     --project-ref uqplppugeickmamycpuz
  npx supabase functions deploy transfer-items-shadow    --project-ref uqplppugeickmamycpuz
  npx supabase functions deploy replay-shadow            --project-ref uqplppugeickmamycpuz   # registry changed
  ```
  Per MIG-008 deploy with placeholder external-service env vars (these shadows construct no client, so MIG-008 is vacuously satisfied — no secrets to strip).
- [ ] **Apply the feature_flags migration** `supabase/migrations/20260516120000_p3_ops_shadow_feature_flags.sql` via the MCP `apply_migration(project_id='uqplppugeickmamycpuz', name, query)` (or `npx supabase db push`). Idempotent; `active_backend` stays `gas` (MIG-007).
- [ ] **Smoke the pipeline**: `POST /functions/v1/replay-shadow` with `{"function_key":"createWillCall"}` (and each of the other 4) using the service_role key; confirm rows land in `public.parity_results` and `feature_flags.mismatch_count_7d` updates for those keys.
