...
> Last updated: 2026-05-16 — **P6 kickoff: 4 payment shadows shipped (compute-only, fleet stays GAS).** `qbo-create-invoice-sb`, `create-stax-invoices-sb`, `run-stax-charges-sb`, `import-iif-sb` + shared GAS-ported helpers `_shared/stax-iif-shadow.ts`. All SHADOW/parity only — no QBO/Stax client constructed in any of them (MIG-008 vacuously satisfied; `EXTERNAL_PAYMENT_CALLS=false` invariant in each; `run-stax-charges-sb` stops at the eligibility boundary before any GET/charge). Migration `20260516000000_p6_payments_shadow_feature_flags.sql` flips `parity_enabled=true` + `shadow_backend='supabase'` for qboCreateInvoice/createStaxInvoices/runStaxCharges and INSERTs a new `importIif` flag — `active_backend` stays `gas`, `tenant_scope` NULL for all four. SHADOW_REGISTRY extended (4 entries; `importIif`→action `importIIF`). Locked-in Opus code review: clean, no Critical/Important, approved. **Parity-compare nuance:** these handlers write no `entity_audit_log`/state — the parity-meaningful artifact is the computed would-be payload in the response body (customer/line-items/totals/eligibility), not a `parity_dryrun` state diff; documented per-function as "structural omissions" (Stax/QBO-API-only steps stay 0). Per MIG-007 fleet stays GAS until the three-layer gate passes later. PR via `feat/migration/p6-payments-shadow`.
...
|---|---|---|---|---|
| `/c/dev/stride-mig-payments` | `feat/migration/p6-payments-shadow` | P6 | qbo/stax/iif payment shadows (compute-only) | 2026-05-16 (remove after PR merge) |
...
| P6 — payments | in_progress | `qboCreateInvoice`, `createStaxInvoices`, `runStaxCharges`, `importIif` | 4 compute-only shadows shipped 2026-05-16 (fleet stays GAS, parity-on). `importIif` added to scope (not in P1.1 substrate; was P4b/P6-tagged). |
...
| `qboCreateInvoice` | gas | 0 | n/a | 0 | n/a | **handler_drafted** | 2026-05-16 (qbo-create-invoice-sb shadow, compute-only, parity-on, unflipped) |
| `createStaxInvoices` | gas | 0 | n/a | 0 | n/a | **handler_drafted** | 2026-05-16 (create-stax-invoices-sb shadow, compute-only, parity-on, unflipped) |
| `runStaxCharges` | gas | 0 | n/a | 0 | n/a | **handler_drafted** | 2026-05-16 (run-stax-charges-sb shadow, eligibility compute-only, parity-on, unflipped) |
| `importIif` | gas | 0 | n/a | 0 | n/a | **handler_drafted** | 2026-05-16 (import-iif-sb shadow; flag INSERTed this PR — not in P1.1 substrate) |
