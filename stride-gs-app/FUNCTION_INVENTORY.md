...
| `handleCompleteShipment_(clientSheetId, payload)` (line 15401) | The "receive a shipment" handler — assigns a new Shipment # via the master RPC, writes one row to Shipments tab, N rows to Inventory tab, optionally creates auto-INSP/ASM tasks per item, and (if receiving billing is enabled) writes RCVG billing rows + any add-on charges. Hyperlinks each new row's Shipment # cell to the shipment folder. Idempotency-protected via a key in shipment notes. Uses a 15-second lock to serialize multiple receivers. Sends SHIPMENT_RECEIVED email with a generated Receiving PDF. | Writes Shipments + Inventory + Tasks + Billing_Ledger sheets, generates PDF via `api_generateDocPdf_`, sends Resend email, calls master RPC for shipment # | P5 |
...
| `doPost` | The Web App POST endpoint — the **atomic counter** for generating IDs. Validates the shared token, takes a script lock, then either: `action="getNextShipmentId"` increments GLOBAL_SHIPMENT_COUNTER and returns `SHP-000123`; or `action="getNextInvoiceId"` (also accepts `getNextInvoiceNo` / `getNextInvoiceNumber`) increments GLOBAL_INVOICE_COUNTER and returns `INV-000123`. Defaults to `getNextShipmentId` if action is absent (v1.x compat). The invoice counter is **the racy counter v38.182 retired** — Supabase `next_invoice_no()` SEQUENCE replaces it. | Reads/writes Master Settings counters. HTTP response. | retiring (the original racy invoice counter — superseded by Postgres `next_invoice_no()` SEQUENCE) |
...
| nextGlobalShipmentNumber_ | Calls the Master spreadsheet's RPC Web App with action `getNextShipmentId` to atomically reserve a globally unique shipment number (so two clients can't end up with the same SHP-* ID). Returns the shipment number string or empty on error. | URL fetch to master RPC. | P2 (helper-misc) |
