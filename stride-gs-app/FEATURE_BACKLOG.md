...
- [x] **Per-item pass/fail toggle UI on RepairDetailPanel — superseded by the BatchWorkItems module (feat/warehouse/batch-work-items, 2026-06-11): per-item Start/Pass/Fail buttons + notes + photos on both RepairDetailPanel and TaskDetailPanel, writes via `update_batch_work_item` RPC (`repair_items.item_status` + legacy `item_result` mirror). Flag-gated to Justin Demo (`batchWorkItems`). Still informational — billing stays at the parent level.**
