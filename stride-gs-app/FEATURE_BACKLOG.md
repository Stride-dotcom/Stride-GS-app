...
- [ ] **[MIGRATION-P3 — NEXT SESSION] `requestRepairQuote` single-item SB cutover** — TaskDetailPanel + ItemDetailPanel still call legacy GAS `handleRequestRepairQuote_`. Wire those buttons to `request-repair-quote-sb` (existing multi-item edge fn, pass `itemIds:[oneItem]`). Add `requestRepairQuote` SHADOW_REGISTRY entry pointing at a tiny pure shadow. ~30 min, follows the cancelRepair → startRepair → sendRepairQuote → respondToRepairQuote template. **Next in cluster after PRs #405-#408.**
- [ ] **[MIGRATION-P4a — NEXT SESSION] `completeRepair` SB cutover** — the big one. SB writes `public.billing` rows directly (authoritative — `public.billing` already supports both per-tenant and consolidated views via `tenant_id` column). New `__writeThroughReverseBilling_` GAS writer for per-tenant Billing_Ledger appends. New `mirrorBillingToCb` GAS endpoint for CB Consolidated_Ledger appends (separate spreadsheet, needs its own doPost case — NOT the per-table writer framework). UPDATE `public.repairs` (status, completed_date, repair_result, final_amount). entity_audit_log (action='complete'). REPAIR_COMPLETE email via Resend. Per MIG-004 status+billing+addons+email are one logical transaction. Standard 14-day canary per MIG-013 since billing is in the path. ~4-5 hrs.
