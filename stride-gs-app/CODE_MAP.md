...
| Edge Functions (P3/P5 ops shadows — pure/Path-C) | `supabase/functions/create-will-call-shadow/index.ts` v1 (action `createWillCall`), `supabase/functions/release-will-call-shadow/index.ts` v1 (flag `releaseWillCall`, action `processWcRelease`), `supabase/functions/create-task-shadow/index.ts` v1 (flag `createTask`, action `batchCreateTasks`), `supabase/functions/release-items-shadow/index.ts` v1 (action `releaseItems`), `supabase/functions/transfer-items-shadow/index.ts` v1 (action `transferItems`, source-tenant `transfer` shape). All read-only, mirror the GAS `api_auditLog_` answer-key dict, no DB writes/no GAS HTTP. Registered in `replay-shadow` SHADOW_REGISTRY (now 11 entries). Migration `20260516120000_p3_ops_shadow_feature_flags.sql` (parity-on, active stays gas). Deploy + SQL pending operator (no SUPABASE_ACCESS_TOKEN in builder env). |
