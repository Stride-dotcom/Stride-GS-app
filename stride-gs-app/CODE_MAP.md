...
| Manual / Add Charge | `src/components/billing/AddChargeModal.tsx` (staff/admin form to add or edit a manual charge; standalone on `Billing.tsx` AND entity-aware when given an `entity` prop — locked client, type-ahead `service_catalog` search w/ class-based rate auto-fill, item picker for multi-item entities, "Miscellaneous" custom path w/ Category dropdown, links the ledger row to the entity + logs a `charge_added` activity entry), `src/components/billing/useAddCharge.tsx` (shared hook returning `{ canAdd, open, modal }` — admin+staff gate + one modal/toast backing BOTH a desktop footer pill AND the mobile FAB action; used by Item/Task/Repair/WillCall/Shipment detail panels + OrderPage so Add Charge appears in every footer and every mobile FAB menu). Backend: `postAddManualCharge`/`AddManualChargePayload` (`src/lib/api.ts`) → `addManualCharge` route → `add-manual-charge-sb` EF or GAS `handleAddManualCharge_`; deterministic ledger id `MANUAL-<SVC>-<ANCHOR>-<YYYYMMDD>` when entity-anchored (collision-suffixed), legacy `MANUAL-<ms>-<rand>` otherwise. |
...
| Edge Functions (P3 cluster — cancelRepair) | `supabase/functions/cancel-repair-shadow/index.ts` v1 (pure parity shadow), `supabase/functions/cancel-repair-sb/index.ts` (SB-primary: status flip + audit + reverse writethrough; PR #750 also captures `status_before_cancel` for reopen) |
| Edge Functions (reopen cancelled repair) | `supabase/functions/reopen-cancelled-repair-sb/index.ts` (PR #750 — SB-first un-cancel: restores `status_before_cancel` or `Pending Quote`, clears the column, `action='reopen'` audit, reverse-writethrough. Invoked directly via `postReopenCancelledRepairSb` — no GAS equivalent, not flag-gated) |
...
| Edge Functions (P3 cluster — sendRepairQuote) | `supabase/functions/send-repair-quote-shadow/index.ts` v1, `supabase/functions/send-repair-quote-sb/index.ts` (server-recomputed totals, 12-column update, REPAIR_QUOTE email via Resend, idempotent re-send; PR #750 sets `quote_revised` + "Revised Repair Quote: …" subject on `isRevision`) |
...
| Lib | `src/lib/api.ts` (apiFetch<T>() typed API client), `src/lib/apiCache.ts` (in-memory cache), `src/lib/supabase.ts` (Supabase client), `src/lib/supabaseQueries.ts` (query helpers), `src/lib/syncEvents.ts` (sync failure tracking), `src/lib/entityEvents.ts` (write-confirmation pub/sub), `src/lib/auditLog.ts` (PR #790 — logEntityAudit() best-effort browser-side entity_audit_log inserts: photo upload/delete, notes, work-order prints), `src/lib/batchLoop.ts` (batched op engine), `src/lib/optimisticBulk.ts` (optimistic UI for bulk), `src/lib/constants.ts`, `src/lib/types.ts` |
| Migrations | `20260415120000_clients_mirror_table.sql` (clients cache), `20260418010000_create_entity_audit_log.sql` (audit log table), `20260420080000_entity_notes_backfill_from_legacy.sql`, `20260420020337_entity_notes_drop_staff_only.sql`, `20260421210000_gs_sync_events_admin_update.sql` (sync event RLS), `20260424200000_entity_audit_log_insert_policy.sql` (insert policy for audit log), `20260612200000_entity_audit_log_performed_by_name.sql` (PR #790 — display-name column for the ActivityTimeline) |
...
| Auto-inspect signal (PR #744, #758) | `complete-shipment-sb` INSP gate: `doInspection = autoInspectionLoaded ? (needsInspection===true) : (needsInspection===true OR clients.auto_inspection)` — the OR is the genuine-race under-inspection guard. `src/pages/Receiving.tsx` sets `autoInspectionLoaded = apiClients.length>0 OR inspectTouchedRef` (PR #758) and a dock-draft hydrate sets `inspectTouchedRef=true` so a reopened-draft submit (clientSheetId from hydrate, apiClients possibly empty) no longer sends a false signal that re-inspects unchecked items. `inspectTouchedRef` + master "INSP all" toggle + `inspectDefault` are PR #744. |
