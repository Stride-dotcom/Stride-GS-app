...
| Test invoice (SB-direct) | `src/lib/api.ts` → `invokeCreateTestStaxInvoice` + `CreateTestStaxInvoiceResult`; `src/pages/Payments.tsx` → Create Test Invoice modal (calls the EF directly, "Push to Stax now" toggle) + `loadData` GAS/noCache path merges SB-only `stax_invoices` rows so a hard refresh never drops the SB-authoritative test invoice. `createTestInvoice` removed from `apiRouter.ts` `GROUPED_STAX_ACTIONS` + `stax-actions-sb` (GAS `handleCreateTestInvoice_` is now legacy/unused). **Chargeability bridge:** the EF best-effort `gasProxy('staxSheetUpsert', …)` mirrors its row into the GAS Stax "Invoices" sheet (`handleStaxSheetUpsert_`, StrideAPI v38.263.0 — idempotent, header-resolved, token-gated, sheet-only no SB re-mirror) so the still-GAS charge path (`handleChargeSingleInvoice_`) can find + charge it (MIG-002; retires when runStaxCharges migrates to SB). |
