  ...
  try {
    ...
    try {
      ...
      if (batchNo) {
        ...
        else if (batchEmail && batchEmail !== 'pending' && batchEmail !== 'notifications_disabled') {
          emailError = batchEmail; // error string
        } else if (batchEmail === 'notifications_disabled') {
...
/**
 * D11 option B — when the last sub-task of a batch goes terminal, send ONE
 * BATCH_COMPLETE summary email with the per-item Result + Notes table.
 * Returns 'pending' (subs still open), 'sent', 'notifications_disabled', or
 * an error string. Duplicate-send safe two ways: the all-terminal check only
 * passes once per batch in practice, and send-email dedupes on
 * idempotencyKey batch-complete:{tenant}:{batchNo} for the race where two
 * subs complete simultaneously.
 */
async function maybeSendBatchSummary(
  ...
  try {
    const sendRes = await fetch(`${supabaseUrl}/functions/v1/send-email`, {
      ...
      body: JSON.stringify({
        ...
        // One summary per batch, ever — also covers two subs completing in
        // the same instant (both pass the all-terminal check).
