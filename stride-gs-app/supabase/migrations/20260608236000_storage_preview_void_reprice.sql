-- ============================================================================
-- Storage preview — voiding a storage invoice makes the storage RE-BILLABLE.
--
-- BUG (void-reprice): voiding a storage invoice for a separate_by_sidemark=FALSE
-- client did NOT free its storage. The blank-sidemark STOR-SUMMARY flips to
-- status='Void' on void, but preview dedup source (b) subtracted summaries with
-- status IN ('invoiced','billed','VOID') — so the voided summary kept its
-- whole-tenant period excluded and the storage could never be re-billed.
--
-- FIX: drop 'void' from source (b)'s status filter so a VOIDED summary no longer
-- excludes its period — matching the authoritative per-item layer (source (a),
-- storage_billing_items), which already treats void as re-billable (it only
-- subtracts 'invoiced'/'billed'). Companion: GAS handleCommitStorageRows_'s
-- finalized-summary gate drops 'void' too (StrideAPI v38.267.0), and the
-- not-yet-live generate_storage_charges RPC (migration 20260608236100).
--
-- Source (a) is the double-bill guard and is UNCHANGED: an item billed via a
-- still-Invoiced summary stays excluded per item. Only genuinely-voided storage
-- reappears (the operator reviews the preview before invoicing). The per-item
-- legacy task-range subtraction (the FIRST v_billed) keeps 'void' — it covers
-- pre-summary legacy STOR rows, not the summary path, and stays conservative.
--
-- This CREATE OR REPLACE is the live body (post-20260608235000) with ONLY the
-- (b) status filter changed: 'invoiced','billed','void' -> 'invoiced','billed'.
...
BEGIN
  ...
  LOOP
    ...
    -- Per-item legacy STOR rows (finalized). Keeps 'void' — legacy per-item
    -- path, not the summary path; conservative.
    v_billed := ARRAY(
    ...
    --     Note: 'invoiced'/'billed' only — a VOID sbi row is re-billable.
    v_billed := v_billed || ARRAY(
    ...
    --     tenant) — FALLBACK ONLY (skipped when the summary has finalized
    --     per-item rows; see the NOT EXISTS). 'void' DROPPED here (was
    --     'invoiced','billed','void'): a VOIDED summary must NOT keep its
    --     period excluded, else voiding a storage invoice can never make the
    --     storage re-billable (it stays stranded). Matches source (a).
    v_billed := v_billed || ARRAY(
