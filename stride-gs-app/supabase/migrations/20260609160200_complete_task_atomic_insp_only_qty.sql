-- complete_task_atomic — multiply by tasks.qty ONLY for inspection tasks.
--
-- Pair migration 20260521210100 made the RPC bill COALESCE(tasks.qty,1) × rate
-- for EVERY task type. But tasks.qty is shared/editable (the BillingPreviewCard
-- qty field is rendered for all task types, and the split workflow stamps
-- grouped_qty), so a non-INSP task carrying qty>1 (observed: a FAB_RUG at
-- qty=16, SPLIT tasks at 2–4) would bill qty×rate on the SB completion path —
-- diverging from the GAS path (which now bills qty 1 for every non-INSP code)
-- and from the product rule that ONLY inspection is per-piece; every other
-- service code is exactly 1 per item ID.
--
-- This recreates the function byte-identical to the live definition EXCEPT:
--   (a) new v_eff_qty — tasks.qty for INSP/Inspection, else 1
...
DECLARE
  ...
  v_total        numeric := 0;
  v_eff_qty      integer := 1;   -- 2026-06-09: per-piece for INSP only, else 1
  v_pt_date      text;
  v_ledger_id    text;
  v_item_notes   text;
