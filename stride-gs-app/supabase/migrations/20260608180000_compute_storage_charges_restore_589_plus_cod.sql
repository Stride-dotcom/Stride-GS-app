-- ============================================================================
-- Restore _compute_storage_charges: #589 dedup + storage_credits + transfer-fix
--                                    PLUS the COD-storage cap
--
-- REGRESSION (introduced 2026-06-05 by the COD migration 20260605170100):
--   That migration did `CREATE OR REPLACE FUNCTION public._compute_storage_charges`
--   built from a VERBATIM COPY of the OLD `20260502200000` body + a COD cap, with
--   its header asserting "non-COD unchanged". But `_compute_storage_charges` had
--   been improved THREE times after 20260502200000:
--     • 20260518042453 — subtract manual `storage_credits` (free periods)
--     • 20260601…(#589) `storage_preview_dedup_summary_transfer` — extend the
--       `v_billed` set with (a) finalized `storage_billing_items` ranges,
--       (b) finalized `STOR-SUMMARY` periods, (c) unbilled `STOR-TRANSFER`
--       backfill ranges; AND the cross-tenant TRANSFER-FIX in the WHERE clause.
--   Because `CREATE OR REPLACE` + the COD migration sorting AFTER all of those,
--   it silently REVERTED every one of them. Live for ALL tenants on the Supabase
--   storage path (the engine behind `calculate_storage_charges` preview +
--   `generate_storage_charges` commit) since 2026-06-05.
--
--   Impact: the Storage-tab preview re-shows already-invoiced storage (the exact
--   #589 bug — actual double-bill still blocked by handleCommitStorageRows_'s
--   independent gate), manual storage credits are no longer honored (real overbill
--   of free days), and a cross-tenant transferred item can match on both tenants.
--
-- FIX: redefine the function as the #589 body (verbatim — the proven-good version
--   that was validated test-copy-diffed across all tenants when #589 shipped) with
--   ONLY the COD additions merged back in:
--     (1) two SELECT columns: cod_storage, cod_storage_start_date
--     (2) the LEAST() cap on v_eff_end after it is resolved
--   Qualifier kept at SECURITY DEFINER + locked search_path to match the current
...
BEGIN
  ...
  FOR r IN
    SELECT
      ...
      -- COD storage (restored from 20260605170100)
  ...
  LOOP
    ...
    -- COD storage cap (restored from 20260605170100): a COD-flagged item is billed
    -- to the designer only through cod_storage_start_date - 1; the remaining days
    -- are collected from the end customer at delivery and must never appear on the
    -- designer's storage report. An item whose whole window is on/after the start
    -- date collapses to v_charge_st > v_charge_en below and is skipped.
...
COMMENT ON FUNCTION public._compute_storage_charges(text, text, date, date) IS
  'Internal shared compute path for calculate_storage_charges + generate_storage_charges. Restores the #589 dedup (storage_billing_items + STOR-SUMMARY + STOR-TRANSFER) + storage_credits subtraction + cross-tenant transfer-fix (silently reverted by the 2026-06-05 COD migration 20260605170100) AND keeps the COD-storage cap (cod_storage_start_date-1). 2026-06-08.';
