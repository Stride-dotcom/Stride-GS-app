-- ============================================================
-- COD Storage — Phase 3 (storage report filtering)
--
-- Re-defines public._compute_storage_charges (the shared inner builder
-- behind calculate_storage_charges + generate_storage_charges) to honor
-- the COD flag:
--
--   • cod_storage = false  → unchanged behavior.
--   • cod_storage = true   → cap the billable window at
--                            (cod_storage_start_date - 1). Days BEFORE
--                            the start date are billed to the designer
--                            normally; days FROM the start date onward
--                            are excluded from the designer's report
--                            (collected from the end customer instead).
--
-- An item whose entire billable window is on/after cod_storage_start_date
-- collapses to v_charge_st > v_charge_en and is skipped via the existing
-- CONTINUE — so it never appears on the designer's storage report.
--
-- This is a verbatim copy of 20260502200000's _compute_storage_charges
-- with ONLY: (1) two new SELECT columns (cod_storage,
-- cod_storage_start_date) and (2) the LEAST() cap after v_eff_end is
-- resolved. Nothing else changed. calculate_storage_charges /
-- generate_storage_charges keep their existing signatures and wrap this.
--
-- Supabase-first note: cod_storage lives only in public.inventory, so the
-- COD filter applies to the Supabase storage path (React Billing → Storage
-- tab + commit-storage-charges-sb recompute). The fleet-wide GAS CB sweep
-- is not COD-aware by design (no GAS changes); Justin Demo runs storage
-- through the SB path.
--
-- 2026-06-05 PST
-- ============================================================
...
BEGIN
  ...
  FOR r IN
    SELECT
      ...
      -- COD storage (Phase 3): cod_storage is a real boolean column,
      -- cod_storage_start_date a real date column.
  ...
  LOOP
    ...
    -- ── COD storage cap (Phase 3) ──────────────────────────────
    -- The designer is billed storage only through the day BEFORE the
    -- end-customer-pays start date. Days from cod_storage_start_date
    -- onward are collected from the end customer at delivery and must
    -- never appear on the designer's storage report.
...
COMMENT ON FUNCTION public._compute_storage_charges(text, text, date, date) IS
  'Internal: shared compute path for calculate_storage_charges + generate_storage_charges. Mirrors GAS handleGenerateStorageCharges_/handlePreviewStorageCharges_ exactly so totals match cell-for-cell. Phase 3 (2026-06-05): caps the billable window at cod_storage_start_date-1 for COD-storage items.';
