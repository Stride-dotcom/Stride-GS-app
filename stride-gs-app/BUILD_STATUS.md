...
**Code-review fix — latent harness bug, fixed here (affects more than this PR):** review caught that `replay-shadow/index.ts` hardcoded `.eq('entity_type','inventory')` when fetching the GAS answer-key audit rows — an updateItem-only-MVP assumption that silently starved EVERY non-inventory shadow of answer-key rows (classifying them all `no_audit_row`). That means the already-shipped repair P3 cluster (`cancelRepair`/`startRepair`/`sendRepairQuote`/`respondToRepairQuote`/`requestRepairQuote`), `completeRepair`, and `completeTask` shadows were registered-but-inert too. Generalized: `ShadowEntry` gains `entity` (per-entry `entity_type`) and `inputShapedChanges` (the partial-redaction skip is updateItem-shaped — keyed on input field names + hardcoded `itemId` — so it's bypassed for fixed/derived-shape shadows that would otherwise be 100% `skip_partial_input`). All 13 registry entries annotated. Net: this PR also unblocks parity scoring for the entire previously-shipped repair/task cluster. Also removed stricter-than-GAS input validation from all 5 new shadows (a shadow that rejects what GAS accepted is scored as a false `shadow_rejected_but_gas_accepted` mismatch — same trap update-item-shadow's header warns about); they now mirror the `api_auditLog_` construction byte-for-byte with zero added validation.
