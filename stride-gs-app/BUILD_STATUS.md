...
> Last updated: 2026-06-01 ([fix/migration/recvship-createtask-parity, this PR] **[MIGRATION-P1.9 / parity hardening] receiveShipment + createTask audit-shape coercion now mirrors GAS's `||` byte-for-byte; documents the 7-day-window healing pattern.** Driven by Justin's observation that `receiveShipment` shows 30 matches + 24 mismatches in `parity_results` rolling-7d — blocking the planned flag-flip to SB. Root-caused via live verification (not DB query — builder env has no service-role): (1) `curl https://uqplppugeickmamycpuz.supabase.co/functions/v1/receive-shipment-shadow` with realistic payloads returns the GAS-matching `{itemCount, carrier}` shape on every test, (2) deployed React bundle `index-BUgfzkA0.js` contains both registry overrides minified verbatim (`completeShipment:{flagKey:'receiveShipment',ef:'receive-shipment-shadow',toAuditShape:e=>({itemCount:...,carrier:...})}`). Same for `create-task-shadow` returning `{summary, svcCodes}`. So no current logic divergence between EF and registry — the 24 mismatches are **legacy data from before [PR #564](https://github.com/Stride-dotcom/Stride-GS-app/pull/564) deployed at 2026-05-29 02:55 PT** (dist commit `e4d7537`). The math checks out: 4 days pre-deploy mismatch (5/25–5/28) + 3 days post-deploy match (5/29–6/01) at ~6–8 calls/day produces the observed 24-mismatch + 30-match split. `mismatch_count_7d` heals automatically when the 7-day window slides past the deploy on 2026-06-05. Same pattern for `createTask`'s 5/5. **Defensive code change shipped this PR** (because letting a mismatch "heal on its own" is not the bar; if a non-string carrier ever hits this code path the silent hash_diff would be invisible to the operator until it bit them): switched [receive-shipment-shadow/index.ts](stride-gs-app/supabase/functions/receive-shipment-shadow/index.ts) and [shadowRegistry.ts → completeShipment](stride-gs-app/src/lib/shadowRegistry.ts) from `String(p.carrier ?? '')` to `(p.carrier as string \| null \| undefined) \|\| ''` — mirrors GAS at [StrideAPI.gs:9598](AppScripts/stride-api/StrideAPI.gs:9598) byte-for-byte. Previous form diverged from GAS only on truthy non-string carriers (number 0, false, NaN, object with `.length`) — never observed in production (every React path goes `carrier.trim()` → string), but a single webhook/retry/integration path passing a non-string would have hash_diff'd silently. Same coercion contract now in EF, registry, and GAS for both `items.length` and `carrier`. `createTask`'s `{summary, svcCodes}` shape already used the correct `Array.isArray ? .join(',') : ''` pattern matching GAS at [StrideAPI.gs:10256](AppScripts/stride-api/StrideAPI.gs:10256) — no code change there, only documentation. MIGRATION_STATUS.md per-function table updated: `receiveShipment` and `createTask` rows now reflect real volume (54 / 10 checks) + "recovering 55% / 50% on rolling 7d, healing by 6/05" annotations. tsc + `npm run build` clean (2,305 modules). **Pending operator actions** (per `feedback_supabase_deploy_token.md` — builder env has no SUPABASE_ACCESS_TOKEN): (1) redeploy `npx supabase functions deploy receive-shipment-shadow --project-ref uqplppugeickmamycpuz` (the EF defensive change), (2) React deploy via dedicated deploy clone (the registry defensive change). Neither is parity-blocking — current EF+registry already match for realistic payloads; this just removes the theoretical drift class.).
