...
## Recent Changes (2026-05-16, [MIGRATION-P6] payment shadows — qbo/stax/iif, compute-only)

`feat/migration/p6-payments-shadow` — P6 (payments) kickoff. Four SHADOW/parity-only Edge Functions that COMPUTE what GAS would push but never call any payment API: `supabase/functions/qbo-create-invoice-sb/` (mirrors `handleQboCreateInvoice_` StrideAPI.gs:41609 — groups Invoiced `public.billing` rows by invoice #, resolves intended QBO customer/sub-job, builds priced line items + NET-terms due dates; tax left null because QBO applies it server-side), `create-stax-invoices-sb/` (`handleCreateStaxInvoices_` :36460 — would-be Stax invoice payloads, subtotal/tax, due-date by client terms), `run-stax-charges-sb/` (`handleRunStaxCharges_` :36817, the real-money path — computes the eligible-to-charge set and STOPS at the eligibility boundary; no GET/charge), `import-iif-sb/` (`handleImportIIF_` :34595 — parses the IIF, computes insert/update/skip/exception decisions). Shared GAS-ported pure helpers in `supabase/functions/_shared/stax-iif-shadow.ts` (date parse, name normalize, line-item build, full IIF parser — Pacific-TZ faithful). State read from `public.billing` / `public.clients` / `public.stax_invoices` / `public.invoice_tracking` mirrors (service-role read only). **MIG-008:** no QBO/Stax/Resend client constructed anywhere — `EXTERNAL_PAYMENT_CALLS=false` invariant in each; vacuously satisfied. Migration `20260516000000_p6_payments_shadow_feature_flags.sql` sets `parity_enabled=true` + `shadow_backend='supabase'` for qboCreateInvoice/createStaxInvoices/runStaxCharges (guarded `WHERE active_backend='gas'`) and INSERTs a new `importIif` flag — **`active_backend` stays `gas`, `tenant_scope` NULL; fleet routes to GAS, nothing live changes.** `replay-shadow` SHADOW_REGISTRY +4 (`importIif`→action `importIIF`). `deno check` clean on all 6 files. Locked-in Opus code review: no Critical/Important, approved (2 low-risk nits noted, not fixed — DST-edge date math that matches GAS for date-only values; NET-30 client-terms default). Per MIG-007 fleet stays on GAS until the three-layer gate passes in a later session. **Edge functions + migration NOT yet applied/deployed if this environment lacks Supabase MCP/`SUPABASE_ACCESS_TOKEN`** — see Pending User Actions.
...
- [ ] **[MIGRATION-P6] Apply the payment-shadow migration + deploy the 4 shadow Edge Functions** (after PR `feat/migration/p6-payments-shadow` merges to `source`). This builder environment had no Supabase CLI / `SUPABASE_ACCESS_TOKEN` / MCP. **Zero live impact** — every flag stays `active_backend='gas'`; this only turns parity logging on and stands up compute-only shadows. From `C:\dev\Stride-GS-app\stride-gs-app` after `git checkout source && git pull`:
  1. Apply migration (Supabase MCP `apply_migration` with the SQL in `supabase/migrations/20260516000000_p6_payments_shadow_feature_flags.sql`, or `npx supabase db push --project-ref uqplppugeickmamycpuz`).
  2. Deploy the four shadows (default JWT verify is fine — they're invoked by the service-role replay-shadow harness, not the browser):
     `npx supabase functions deploy qbo-create-invoice-sb create-stax-invoices-sb run-stax-charges-sb import-iif-sb --project-ref uqplppugeickmamycpuz`
     (also redeploy `replay-shadow` so its new SHADOW_REGISTRY entries are live: `npx supabase functions deploy replay-shadow --project-ref uqplppugeickmamycpuz`.)
  3. No new secrets. MIG-008 is vacuously satisfied — these functions never read a QBO/Stax/Resend key (none is constructed); the only client is the built-in Supabase service-role reader. Optional sanity: `POST /functions/v1/replay-shadow {"function_key":"qboCreateInvoice"}` and confirm rows land in `parity_results`.
