...
- [ ] **🚨 DEPLOY-BLOCKING: Apply migration `20260519130000_dt_orders_taxable_subtotal.sql`** (fix/billing/do-modal-taxable-services) **BEFORE the React `npm run deploy` for this change.** The new React code writes `taxable_subtotal` into every `dt_orders` insert/update; until the column exists, PostgREST rejects the payload (`PGRST204`) and **all delivery-order creation/edit fails**. The PR is merged to `source` but the React deploy is intentionally HELD until this is applied. Builder env has no Supabase MCP / `SUPABASE_ACCESS_TOKEN`. Apply via Supabase MCP `apply_migration(project_id='uqplppugeickmamycpuz', name='dt_orders_taxable_subtotal', query=<file contents>)`, or paste the file into the Supabase SQL editor (idempotent, `ADD COLUMN IF NOT EXISTS`, safe to re-run). Verify: `SELECT column_name FROM information_schema.columns WHERE table_name='dt_orders' AND column_name='taxable_subtotal';` returns one row. THEN deploy React: from `C:\dev\Stride-GS-app\stride-gs-app` run `npm run deploy -- "fix(billing): tax only taxable=true accessorials, not whole DO subtotal"`.
