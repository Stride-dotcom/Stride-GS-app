...
## Recent Changes (2026-05-16, [MIGRATION-P4a] createInvoice/voidInvoice/generateUnbilledReport shadows — PR pending)

`feat/migration/p4a-invoice-shadow` (worktree `/c/dev/stride-mig-invoice`) — three SHADOW/parity Edge Functions porting the COMPUTE half of the GAS billing handlers so the parity harness can diff SB-would-produce vs live GAS, **with zero live impact**: `supabase/functions/create-invoice-sb/index.ts` (ports `handleCreateInvoice_` line-item build / STOR grouping / `api_money_` / `api_buildInvoiceLineItemsHtml_` / `api_esc_` / sidemark assertion / PDF tokens / QB customer name), `supabase/functions/void-invoice-sb/index.ts` (which `public.billing` rows would flip Invoiced→Void), `supabase/functions/generate-unbilled-report-sb/index.ts` (cross-tenant Unbilled row-set from `public.billing`). All three are pure reads — no `.insert/.update/.delete/.upsert`, no PDF, no email, no CB write, and critically **no `next_invoice_no()` consumption** (returns placeholder `SHADOW-NOT-ASSIGNED`; atomic-counter landmine respected). Migration `supabase/migrations/20260516120000_p4a_invoice_shadow_feature_flags.sql` sets `parity_enabled=true, shadow_backend='supabase'` on `createInvoice`/`voidInvoice` guarded `WHERE active_backend='gas'` (same idiom as `20260514210100_complete_task_feature_flag.sql`) and INSERTs a net-new `generateUnbilledReport` row. **`active_backend` stays `gas` fleet-wide — nothing cut over.** Parity posture is MIG-007 layer-1 (per-call live shadow — caller diffs the returned `parity` object); layer-2 historical replay (SHADOW_REGISTRY/replay-shadow) intentionally NOT wired because invoice creation has no fixed audit-changes shape (documented in the migration + MIGRATION_STATUS.md). Code review: registered `code-reviewer` subagent type unavailable in this harness (same as #443/#444) — ran the `.claude/agents/code-reviewer.md` definition verbatim via an Opus general agent. One Critical found + fixed: ported `esc()` had an extra `'`→`&#39;` replacement that GAS `api_esc_` (StrideAPI.gs:28877) does NOT do, which would false-mismatch every invoice line containing an apostrophe; now byte-faithful (`String(s||'')`, `[&<>"]` only). Known parity caveats logged in MIGRATION_STATUS.md (PAYMENT_TERMS source; exotic date-string normalization).

**Pending user actions:** this builder environment has **no Supabase MCP / `SUPABASE_ACCESS_TOKEN`** and no `supabase` CLI, so the migration is **NOT applied** and the Edge Functions are **NOT deployed** (committed only). After PR merge, Justin runs:
1. Apply the feature_flags migration (Supabase MCP `apply_migration(project_id='uqplppugeickmamycpuz', name='20260516120000_p4a_invoice_shadow_feature_flags', query=<file contents>)`, or `supabase db push`).
2. Deploy the three Edge Functions (MIG-008 stripped-credential posture is vacuously satisfied — they make no external calls; they DO need `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY`/`SUPABASE_ANON_KEY` for reads + auth):
   `supabase functions deploy create-invoice-sb && supabase functions deploy void-invoice-sb && supabase functions deploy generate-unbilled-report-sb`
3. Verify: `SELECT function_key, active_backend, shadow_backend, parity_enabled FROM public.feature_flags WHERE function_key IN ('createInvoice','voidInvoice','generateUnbilledReport');` — expect all `active_backend='gas'`, `shadow_backend='supabase'`, `parity_enabled=true`.
