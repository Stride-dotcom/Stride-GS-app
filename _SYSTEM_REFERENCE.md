...
- **⚠️ WORKTREE SOURCE SYNC RULE: NEVER deploy a bundle built from a worktree without FIRST merging the worktree branch into source.** If you build from a worktree and push to dist, any subsequent build from the main workspace (which tracks source) will overwrite your changes with the older source code. **CORRECT FLOW:** (1) Work in worktree → commit → push branch → create PR → merge to source. (2) Pull source into main workspace. (3) Build from main workspace. (4) Deploy from main workspace dist repo. **NEVER:** Build in worktree → push directly to dist (bypasses source, causes silent reverts). **Root cause in session 83-ish:** 23 worktree commits lived behind directly-pushed dist bundles for a week; when the password PR merged to source was built from main workspace, its bundle (lacking those 23 commits) overwrote the dist and users saw every entity-page redesign silently revert. The worktree branch was still fine — just not merged. Always merge before deploying from any workspace other than the one that built the bundle.
