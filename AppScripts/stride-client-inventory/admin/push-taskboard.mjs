/**
 * push-taskboard.mjs
 * Pushes the Task Board script to its Apps Script project.
 * Run from: stride-client-inventory/ directory
 * Usage: node admin/push-taskboard.mjs
 */
...
// Task Board Apps Script project ID (from CLAUDE.md)
const SCRIPT_ID = '1RgsXWnAfZfpU5M58SE19ZFf7cuh0HtC5eUMZ86IxQ2jQwL5Pl5UJZvMg';

// Task Board is a single file
const LOCAL_FILE = join(__dirname, '..', '..', 'task board script.txt');
...
async function main() {
  ...
  // Read local file
  console.log('Reading Task Board script...');
  const source = readFileSync(LOCAL_FILE, 'utf8');
  console.log(`  task board script.txt → Code (${(source.length / 1024).toFixed(1)} KB)`);
  ...
  console.log('');
  console.log('Next: Bound script — no deploy step needed. Changes are live immediately.');
