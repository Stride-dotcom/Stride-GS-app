/* ===================================================
   Triggers.gs — v4.8.2 — 2026-06-10 PST — [BILLING] fix: processTaskCompletionById_ (sheet-checkbox task completion) now bills RUSH tasks at the inventory item's TRUE qty too, joining INSP — a rush inspection of a carton of 6 pieces bills "Rush × 6" not "× 1". Gate broadened from (svcCode === INSP) to (svcCode === INSP OR svcCode === RUSH); every other svc code still stays 1-per-ID. Mirrors handleCompleteTask_ in StrideAPI.gs v38.272.0 so the React-app path and the direct-sheet path agree.
   v4.8.1 — 2026-06-09 PST — [BILLING] fix: processTaskCompletionById_ (sheet-checkbox task completion) now bills INSPECTION tasks at the inventory item's TRUE qty (Qty × rate) instead of a hardcoded Qty 1 — a carton of 6 inspected pieces now bills "Inspection × 6" not "× 1". Scoped to INSP only; every other svc code stays 1-per-ID. Mirrors handleCompleteTask_ in StrideAPI.gs v38.271.0 so the React-app path and the direct-sheet path agree.
...
/* =====================================================================
   ...
   ===================================================================== */
var SHARED_HANDLER_VERSION = "1.2.0"; // 1.2.0: repair WO items table adds Location column ({{ITEM_LOCATION}})
...
function SH_generateRepairWorkOrderPdf_(ss, repairRowData, repairMap, folderUrl) {
  ...
  var tokens = {
    ...
    "{{ITEM_DESC}}": e(itemDesc), "{{ITEM_SIDEMARK}}": e(sidemark), "{{ITEM_ROOM}}": e(itemRoom), "{{ITEM_LOCATION}}": e(itemLoc),
...
function processTaskCompletionById_(srcSs, taskId, resultValue) {
  ...
  try {
    ...
    if (shouldBill) {
      ...
      // INSPECTION and RUSH bill the inventory item's TRUE qty (carton of 6
      // pieces → Qty 6 × rate); a rush inspection scales per-piece just like a
      // normal one. Every other svc code stays Qty 1 per item ID. Mirrors
      // handleCompleteTask_ in StrideAPI.gs v38.272.0. SH_writeBillingRow_
      ...
      var billQty = isPerPieceSvc ? (Number(invItem.qty) > 0 ? Math.round(Number(invItem.qty)) : 1) : 1;
