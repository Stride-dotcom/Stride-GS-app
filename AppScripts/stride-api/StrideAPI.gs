/* ===================================================
   ...
   (1) api_appendClientRow_ now writes to ALL columns with the same header name (freshHMapAll multi-index map). Root cause: Justin's CB Clients sheet has DUPLICATE "Web App URL" columns. Previous code wrote to the LAST occurrence (per JS object semantics), but handleGetClients_ used sheetToObjects_ which might read the other duplicate, causing the React Edit modal to show an empty Web App URL even though the deploy succeeded. Now writes hit all duplicates. Also surfaces the duplicate headers as a warning so the user knows to clean them up.
   (2) handleFinishClientSetup_ now LIST's existing deployments via the Apps Script API first, and REUSES any existing Web App deployment instead of creating a new one. Fixes two problems: (a) Arkitektura Forty already has a working deployment but the URL never landed in CB Clients — reuse reads the URL from the existing deployment and writes it to all duplicate columns. (b) Clicking Finish Setup multiple times no longer creates orphan deployments for the same script.
   ...
   (2) handleFinishClientSetup_: 4-strategy lookup: (a) CB Clients Script ID column, (b) client sheet's Settings tab _SCRIPT_ID row (written automatically whenever any Stride Admin menu action runs — verifyTriggers writes ScriptApp.getScriptId() there), (c) Drive search within client folder by name, (d) legacy sheet-parent search. Strategy (b) is the MOST RELIABLE because the bound script writes its own ID after any user action on the sheet.
   ...
   v38.34.0: (1) handleOnboardClient_ now retries script ID discovery via Drive search with backoff delays (0, 1s, 3s, 5s, 7s — total max wait ~16s). Drive indexing lag after makeCopy() typically resolves within 2-8 seconds, so most onboardings will now auto-deploy the Web App without needing Finish Setup. (2) All outdated CLI warning messages ("run npm run sync && npm run deploy-clients", "run npm run install-triggers") replaced with instructions to click the in-app "Finish Setup" button. Justin's team can't run npm commands, so pointing them at the CLI was a dead end.
   ...
   v38.31.0: (1) New handleFinishClientSetup_ endpoint for recovering partially-onboarded clients where the Web App was never deployed (Drive indexing lag, transient failures). Takes clientSheetId, re-runs script ID discovery + Web App deploy + trigger install. Idempotent. Powers the in-app "Finish Setup" button. Replaces the "npm run sync && npm run deploy-clients" CLI fallback. (2) api_appendClientRow_ rewritten for robustness: re-reads header row fresh from sheet (no stale snapshot), writes each cell individually (one bad cell no longer kills others), logs missing columns, includes Auto Charge field (was missing). Fixes "only Client Name + Email saved, everything else blank" bug that occurred when the header snapshot was out of sync with the actual sheet.
   ...
   v38.29.0: api_appendClientRow_ now calls range.clearDataValidations() before setValues(), so stale dropdown rules on the CB Clients "Payment Terms" column (e.g. ALL-CAPS values) can't block the row write. Previously, the setValues() throw was caught as a silent warning and the client row was never appended, leaving a "ghost" client (spreadsheet exists but not in the Clients tab). The validation bypass only affects the row being written, not the header row.
   ...
   v38.27.0: handleRemoteAction_ now sends REMOTE_EXEC_TOKEN ("stride-remote-exec-9f3a2") instead of API_TOKEN. Clients' RemoteAdmin.gs doPost checks against their hardcoded REMOTE_EXEC_TOKEN_ constant, not the API_TOKEN property. Previously all clients (except Master Inventory Template for historical reasons) returned "unauthorized" on updateHeaders / installTriggers. Configurable via CLIENT_REMOTE_EXEC_TOKEN Script Property if token ever rotates.
   StrideAPI.gs — v38.26.0 — 2026-04-10 — handleRemoteAction_ error propagation
   v38.26.0: handleRemoteAction_ now populates BOTH `message` and `error` fields on failed results so React's `r.error` display works. Previously on HTTP 200 + body.ok=false, the failure reason landed in `message` but React only read `error`, producing empty error lines. Also falls back to raw truncated body if body.message is empty (surfaces HTML error pages etc). Added sheetId to each result for proper React key usage.
   StrideAPI.gs — v38.25.0 — 2026-04-10 — Transfer Date cutover for storage billing
   v38.25.0: handleTransferItems_ accepts transferDate (YYYY-MM-DD, defaults to today, past allowed, future rejected). New "Transfer Date" column auto-added to source + destination Inventory tabs (api_ensureColumn_ helper). Storage billing splits at the cutover: source bills receive→(transferDate-1), destination bills transferDate→endDate with FRESH free-days credit. Released items still skipped if no Release Date. React TransferItemsModal exposes Transfer Date input (defaults today, max today). No backfill needed (no live transferred items yet).
   v38.24.0: handleGenerateStorageCharges_ + handlePreviewStorageCharges_ now include Released and On Hold items in storage billing for the days they were physically in stock. Released items require a Release Date (skipped with warning otherwise). Existing release-date math (-1 day convention) handled the calculation correctly — only the over-restrictive status filter needed to change. Transferred items still excluded (destination client inherits via transfer flow).
...
function api_fullClientSync_(tenantId, entityTypes) {
  try {
    ...
    if (!url || !key) return;
    var ss = SpreadsheetApp.openById(tenantId);
    // Resolve client name from Settings tab for task sync
    var clientName = "";
    try {
      ...
      switch (type) {
        ...
        case "task":
          ...
          if (taskSheet) {
            ...
            // Read folder URLs from Task ID hyperlinks + shipment folder map
            var taskFolderUrls = api_readIdFolderUrls_(taskSheet, "Task ID");
            ...
            for (var j = 0; j < taskRows.length; j++) {
              ...
              taskSb.push(sbTaskRow_(tenantId, {
                ...
                vendor: taskRows[j]["Vendor"] || "", sidemark: taskRows[j]["Sidemark"] || "",
                shipmentNumber: tShipNo,
                cancelledAt: formatDate_(taskRows[j]["Cancelled At"]),
                startedAt: formatDate_(taskRows[j]["Started At"]),
        ...
        case "shipment":
          ...
          if (shipSheet) {
            ...
            // Read folder URLs from Shipment # hyperlinks
        ...
        case "repair":
          ...
          if (repSheet) {
            ...
            // Read folder URLs from Repair ID hyperlinks + shipment/task folder maps
            var repFolderUrls = api_readIdFolderUrls_(repSheet, "Repair ID");
...
function doGet(e) {
  try {
    ...
    // QBO OAuth callback — no token required (browser redirect from Intuit)
    // Detected by presence of "code" + "state" params (QBO doesn't allow ?action= in redirect URI)
    if (params.code && params.state && !action) {
      var qboCode  = String(params.code    || "").trim();
      var qboState = String(params.state   || "").trim();
      var qboRealm = String(params.realmId || "").trim();
      var cbResult = qbo_exchangeCode_(qboCode, qboState, qboRealm);
      return HtmlService.createHtmlOutput(
        '<html><body><script>' +
        'window.opener && window.opener.postMessage(' + JSON.stringify(cbResult) + ', "*");' +
        'setTimeout(function(){ window.close(); }, 1500);' +
        '</script>' +
        '<p style="font-family:Inter,sans-serif;text-align:center;margin-top:60px;font-size:16px;">' +
        (cbResult.success ? '&#10003; Connected to QuickBooks! You may close this window.' : '&#10007; Connection failed: ' + (cbResult.error || 'Unknown error')) +
        '</p></body></html>'
      );
    }
    ...
    switch (action) {
      ...
      // ─── QBO Integration (admin-only reads) ───
...
function handleGetClients_(scopeIds) {
  ...
  // v38.39.0 — normalize headers to UPPERCASE at build time so every field lookup
  // below is immune to header-case drift in the sheet. Matches the hMap convention
  // already used by api_clientRowToPayload_ and api_updateClientRow_.
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    for (var j = 0; j < headersUpper.length; j++) {
      if (headersUpper[j]) obj[headersUpper[j]] = row[j];
    }
...
/**
 * Fetch a single task by exact Task ID. Read-only fallback endpoint for
 * standalone task detail page when Supabase cache misses.
 * On success, upserts the task into Supabase for next-time fast reads.
 */
function handleGetTaskById_(clientSheetId, params) {
  var targetTaskId = String(params.taskId || "").trim();
  if (!targetTaskId) return errorResponse_("Missing taskId parameter", "INVALID_PARAMS");

  var ss = SpreadsheetApp.openById(clientSheetId);
  var sheet = ss.getSheetByName("Tasks");
  if (!sheet) return errorResponse_("Tasks sheet not found", "NOT_FOUND");

  var rows = sheetToObjects_(sheet);
  var match = null;
  for (var r = 0; r < rows.length; r++) {
    if (String(rows[r]["Task ID"] || "").trim() === targetTaskId) { match = rows[r]; break; }
  }
  ...
  // Folder URLs
  var taskFolderUrls = api_readIdFolderUrls_(sheet, "Task ID");
  var shipFolderMap = api_buildShipmentFolderMap_(ss);
  ...
  var task = {
    ...
    status: String(match["Status"] || "Open").trim(),
    itemId: taskItemId,
    vendor: vendor,
    description: description,
    ...
    completedAt: formatDateTime_(match["Completed At"]),
    cancelledAt: formatDateTime_(match["Cancelled At"]),
    result: String(match["Result"] || "").trim(),
    taskNotes: String(match["Task Notes"] || "").trim(),
    svcCode: String(match["Svc Code"] || "").trim(),
    billed: toBool_(match["Billed"]),
    assignedTo: String(match["Assigned To"] || "").trim(),
    startedAt: formatDateTime_(match["Started At"]),
    customPrice: toNum_(match["Custom Price"]) || undefined,
    taskFolderUrl: taskFolderUrls[targetTaskId] || "",
    shipmentFolderUrl: shipFolderMap[shipNo] || ""
  };
...
/**
 * Non-destructively ensures a column header exists on a sheet. Appends to the
 * end if missing. Returns the 1-based column index. Used by transfer/storage
 * billing flows that need to read or write the "Transfer Date" column without
 * waiting for a maintenance run on every client sheet.
 *
 * v38.40.0 — Case-insensitive match + duplicate guard. Previously this helper
 * did a case-sensitive lookup via api_getHeaderMap_, so if a sheet already had
 * the column in a different case (e.g. "TRANSFER DATE" instead of "Transfer Date"),
 * the lookup would miss and a duplicate column would be appended. Now the match
 * is case-insensitive: if a column with the same name (any case) exists, return
 * its existing index without touching the header row. Only truly-missing columns
 * trigger the auto-create branch, and when they do, the header is written using
 * the passed `headerName` VERBATIM — so callers should pass the canonical case
...
function handleOnboardClient_(payload) {
  ...
  // v38.35.0: Container-bound scripts do NOT reliably show up as children of
  // their spreadsheet in Drive's parent-child queries. The old approach (search
  // for scripts with spreadsheetId as parent) worked sporadically when Drive
  // happened to index the association, but failed most of the time.
  //
  // New approach: search the PARENT FOLDER (client folder) for any Apps Script
  // project whose name matches the client sheet. When makeCopy() duplicates a
  ...
  for (var retryIdx = 0; retryIdx < retryDelays.length; retryIdx++) {
    ...
    try {
      var folderScripts = DriveApp.searchFiles(
        "'" + clientFolderId + "' in parents and mimeType = 'application/vnd.google-apps.script' and trashed = false"
      );
      while (folderScripts.hasNext()) {
        var candidate = folderScripts.next();
        // Pick the one named like the sheet (safer than just "first result")
        if (candidate.getName().indexOf(clientName) !== -1 || folderScripts.hasNext() === false) {
          newScriptId = candidate.getId();
          Logger.log("handleOnboardClient_: Discovered script ID via folder search on attempt " + (retryIdx + 1) + ": " + newScriptId + " (name: " + candidate.getName() + ")");
          break;
        }
      }
      if (newScriptId) break;
    } catch (folderErr) {
      Logger.log("handleOnboardClient_: Folder script search failed on attempt " + (retryIdx + 1) + ": " + folderErr);
    }
    // Try 2 (legacy): search by sheet parent — unreliable but try anyway
    try {
      ...
      if (scriptFiles.hasNext()) {
        ...
        Logger.log("handleOnboardClient_: Discovered bound script ID via sheet-parent search on attempt " + (retryIdx + 1) + ": " + newScriptId);
        break;
      }
    } catch (scriptErr) {
      Logger.log("handleOnboardClient_: Sheet-parent script search error on attempt " + (retryIdx + 1) + ": " + scriptErr);
    }
  }
  if (!newScriptId) {
  ...
  // v38.29.0: Row-write failure is now reported as a visible warning (was silently caught).
  // v38.33.0: Capture the returned rowInfo so later writes (Web App URL) can
  // target the exact row without a race-prone re-query.
  var rowWarning = null;
  var clientRowInfo = null;
  try {
    clientRowInfo = api_appendClientRow_(clientsSh, clientsHMap, clientsHeader.length, {
      clientName:            clientName,
      ...
      // v38.32.0: Use ?? pattern so explicit 0 isn't coerced to 30. Previously
      // `Number(payload.freeStorageDays) || 30` treated 0 as falsy and silently
      // replaced it with 30, overriding what the user entered.
      freeStorageDays:       payload.freeStorageDays != null && payload.freeStorageDays !== '' ? Number(payload.freeStorageDays) : 0,
      discountStoragePct:    payload.discountStoragePct != null && payload.discountStoragePct !== '' ? Number(payload.discountStoragePct) : 0,
      discountServicesPct:   payload.discountServicesPct != null && payload.discountServicesPct !== '' ? Number(payload.discountServicesPct) : 0,
      ...
      autoCharge:            payload.autoCharge === true,
      notes:                 String(payload.notes || ""),
      shipmentNote:          String(payload.shipmentNote || ""),
    ...
    try {
      ...
      if (versionNumber) {
        ...
        if (newWebAppUrl) {
          ...
          // v38.36.0: Write Web App URL + Deployment ID to ALL matching columns
          // (some sheets have duplicate "Web App URL" columns). Uses the multi-index
          // map returned from api_appendClientRow_. Falls back to the single-index
          // map if multi-index isn't available (back-compat).
          try {
            if (clientRowInfo && clientRowInfo.targetRow) {
              var freshHMapAll = clientRowInfo.freshHMapAll || {};
              var freshHMapSingle = clientRowInfo.freshHMap || clientsHMap;

              // Helper to write to ALL columns with the given header name
              function writeToAllMatching_(headerName, value) {
                var indices = freshHMapAll[headerName];
                if (!indices || indices.length === 0) {
                  // Fallback to single-index map
                  var singleIdx = freshHMapSingle[headerName];
                  if (singleIdx !== undefined) indices = [singleIdx];
                }
                if (!indices || indices.length === 0) {
                  return 0;
                }
                ...
                for (var wi = 0; wi < indices.length; wi++) {
                  try {
                    clientsSh.getRange(clientRowInfo.targetRow, indices[wi] + 1).setValue(value);
                    written++;
                  } catch (_) {}
                }
                Logger.log("handleOnboardClient_: Wrote " + headerName + " to " + written + " column(s) in row " + clientRowInfo.targetRow);
                return written;
              }

              var webUrlWrites = writeToAllMatching_("WEB APP URL", newWebAppUrl);
              if (webUrlWrites === 0) {
                warnings.push("Web App URL column not found in CB Clients header — skipped write");
              }

              if (deployData.deploymentId) {
                var deployIdWrites = writeToAllMatching_("DEPLOYMENT ID", deployData.deploymentId);
                if (deployIdWrites === 0) {
                  warnings.push("Deployment ID column not found in CB Clients header — skipped write");
                }
              }

              if (clientRowInfo.duplicateHeaders && clientRowInfo.duplicateHeaders.length > 0) {
                warnings.push("CB Clients sheet has duplicate columns: " + clientRowInfo.duplicateHeaders.join(", ") + ". Consider deleting the duplicates to keep the sheet clean. Data was written to all copies for safety.");
              }
            } else {
              warnings.push("Web App deployed but client row wasn't written successfully — Web App URL not saved to CB Clients tab");
            }
          } catch (writeErr) {
            Logger.log("handleOnboardClient_: Write Web App URL to CB failed: " + writeErr);
            warnings.push("Web App deployed but failed to write URL to CB Clients: " + writeErr.message);
          }
          ...
          try {
            ...
            warnings.push("Trigger install failed — click the 'Finish Setup' button on this client's card in Settings → Clients to retry.");
          }
        } else {
          Logger.log("handleOnboardClient_: Deployment created but no Web App URL in response");
          warnings.push("Web App deployment created but URL not found in response — click the 'Finish Setup' button on this client's card in Settings → Clients to retry.");
        }
      } else {
        Logger.log("handleOnboardClient_: Version creation failed: " + versionRes.getContentText());
        warnings.push("Auto-deploy failed (version creation) — click the 'Finish Setup' button on this client's card in Settings → Clients to retry.");
      }
    } catch (deployErr) {
      Logger.log("handleOnboardClient_: Auto-deploy failed (non-fatal): " + deployErr);
      warnings.push("Auto-deploy failed: " + deployErr.message + " — click the 'Finish Setup' button on this client's card in Settings → Clients to retry.");
    }
  } else {
    // No script ID found after retries — user can trigger Finish Setup once Drive catches up
    warnings.push("Web App not auto-deployed yet (Drive indexing lag). Wait ~30 seconds, then click the yellow 'Finish Setup' button on this client's card in Settings → Clients to complete setup.");
  }
  ...
  // Send internal notification email — FYI with authorize instructions
  try {
    var sheetUrl = "https://docs.google.com/spreadsheets/d/" + newSpreadsheetId + "/edit";
    var notifSubject = 'New Client Onboarded: "' + clientName + '" — authorize the spreadsheet';
    var notifHtml = '<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="margin:0;padding:0;background:#F8FAFC;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Arial,sans-serif;">'
      + '<div style="max-width:640px;margin:0 auto;padding:24px;">'
      + '<div style="background:#000000;border-radius:12px 12px 0 0;padding:16px 22px;">'
      + '<table width="100%" cellpadding="0" cellspacing="0" border="0"><tr>'
      + '<td style="vertical-align:middle;">'
      + '<img src="https://static.wixstatic.com/media/a38fbc_e4bdb945b21f4b9f8b10873799e8f8f1~mv2.png" width="34" height="34" style="vertical-align:middle;margin-right:10px;" />'
      + '<span style="color:#ffffff;font-size:16px;font-weight:700;vertical-align:middle;">Stride Logistics </span>'
      + '<span style="color:#E85D2D;font-size:16px;font-weight:700;vertical-align:middle;">WMS</span></td>'
      + '<td style="text-align:right;vertical-align:middle;"><span style="color:#ffffff;font-size:14px;font-weight:700;">New Client Onboarded</span></td>'
      + '</tr></table></div>'
      + '<div style="background:#ffffff;border:1px solid #E2E8F0;border-top:none;border-radius:0 0 12px 12px;padding:22px;">'
      + '<h1 style="font-size:20px;font-weight:900;color:#1E293B;margin:0 0 6px 0;">Client Onboarded: ' + clientName.replace(/</g, "&lt;") + '</h1>'
      ...
      + '<span style="font-size:14px;font-weight:800;color:#1E293B;">' + clientName.replace(/</g, "&lt;") + '</span></td></tr>'
      + '<tr><td style="padding:4px 0;"><span style="font-size:11px;text-transform:uppercase;font-weight:800;color:#64748B;">Client Email</span><br/>'
      + '<span style="font-size:14px;color:#1E293B;">' + clientEmail + '</span></td></tr>'
      ...
      + (newWebAppUrl ? '<tr><td style="padding:4px 0;"><span style="font-size:11px;text-transform:uppercase;font-weight:800;color:#64748B;">Web App</span><br/><span style="font-size:12px;color:#16A34A;font-weight:700;">Deployed automatically</span></td></tr>' : '')
      + '</table></div>'
      // Authorize step
      ...
      + '</ol>'
      + '<p style="font-size:13px;color:#64748B;margin:12px 0 0 0;">That\'s it! The client sheet is now fully operational.</p>'
      + '</div>'
      // Auto-configured checklist
      + '<div style="background:#F1F5F9;border:1px solid #E2E8F0;border-radius:10px;padding:14px;margin-bottom:14px;">'
      + '<h2 style="font-size:14px;font-weight:800;color:#E85D2D;margin:0 0 10px 0;text-transform:uppercase;letter-spacing:1px;">Auto-Configured</h2>'
      + '<table cellpadding="0" cellspacing="0" border="0" width="100%" style="font-size:13px;color:#1E293B;line-height:1.7;">'
      + '<tr><td style="padding:4px 0;color:#16A34A;font-weight:700;width:24px;">&#10003;</td><td>Client spreadsheet created from template</td></tr>'
      + '<tr><td style="padding:4px 0;color:#16A34A;font-weight:700;">&#10003;</td><td>Drive folders created (Photos, Invoices)</td></tr>'
      + '<tr><td style="padding:4px 0;color:#16A34A;font-weight:700;">&#10003;</td><td>All settings written to client sheet</td></tr>'
      + '<tr><td style="padding:4px 0;color:#16A34A;font-weight:700;">&#10003;</td><td>Client row added to Consolidated Billing</td></tr>'
      + '<tr><td style="padding:4px 0;color:#16A34A;font-weight:700;">&#10003;</td><td>User account created (Active)</td></tr>'
      + (newWebAppUrl
        ? '<tr><td style="padding:4px 0;color:#16A34A;font-weight:700;">&#10003;</td><td>Web App deployed &amp; triggers installed</td></tr>'
        : '<tr><td style="padding:4px 0;color:#DC2626;font-weight:700;">&#10007;</td><td>Web App not auto-deployed &mdash; see warnings</td></tr>')
      + '</table></div>'
      // Warnings
      + (warnings.length > 0
        ? '<div style="background:#FEF3C7;border:1px solid #F59E0B;border-radius:10px;padding:14px;margin-bottom:14px;">'
          + '<h2 style="font-size:14px;font-weight:800;color:#92400E;margin:0 0 8px 0;">Warnings</h2>'
          + '<ul style="font-size:13px;color:#92400E;margin:0;padding-left:18px;line-height:1.7;">'
          + warnings.map(function(w) { return '<li>' + w.replace(/</g, "&lt;") + '</li>'; }).join('')
          + '</ul></div>'
        : '')
      // Footer
      + '<div style="border-top:1px solid #E2E8F0;padding-top:14px;margin-top:10px;font-size:12px;color:#64748B;text-align:center;">'
      + 'Stride Logistics &middot; Kent, WA &middot; whse@stridenw.com</div>'
      + '</div></div></body></html>';

    GmailApp.sendEmail(
      "email@stridenw.com",
      notifSubject,
      "New client \"" + clientName + "\" onboarded via Stride app. Open the spreadsheet and authorize to complete setup.",
      { htmlBody: notifHtml, from: "whse@stridenw.com", name: "Stride Logistics WMS" }
    );
    Logger.log("handleOnboardClient_: internal notification email sent for " + clientName);
  } catch (emailErr) {
    Logger.log("handleOnboardClient_: notification email failed (non-fatal): " + emailErr);
  }
...
/**
 * finishClientSetup — Recovery endpoint for clients where onboarding partially
 * completed. Common scenario: Drive folders + spreadsheet were created, but the
 * bound script ID wasn't discoverable immediately (Drive indexing lag), so the
 * Web App deployment step was skipped. This endpoint re-runs ONLY the Web-App
 * deploy + trigger install steps, without re-creating anything. Idempotent.
 *
...
function handleFinishClientSetup_(payload) {
  ...
  // Validate that this client exists in CB Clients
  var cbId = prop_("CB_SPREADSHEET_ID");
  if (!cbId) return errorResponse_("CB_SPREADSHEET_ID not configured", "CONFIG_ERROR");
  ...
  var nameColIdx     = cbHdr.indexOf("CLIENT NAME");
  var idColIdx       = cbHdr.indexOf("CLIENT SPREADSHEET ID");
  var scriptIdColIdx = cbHdr.indexOf("SCRIPT ID");
  var webAppUrlColIdx = cbHdr.indexOf("WEB APP URL");
  var deployIdColIdx = cbHdr.indexOf("DEPLOYMENT ID");

  if (idColIdx < 0) return errorResponse_("CB Clients tab missing 'Client Spreadsheet ID' column", "CONFIG_ERROR");

  var clientRowNum = -1;
  var clientName = "";
  for (var i = 1; i < clientsData.length; i++) {
    if (String(clientsData[i][idColIdx] || "").trim() === clientSheetId) {
      clientRowNum = i + 1; // 1-based sheet row
      clientName = nameColIdx >= 0 ? String(clientsData[i][nameColIdx] || "").trim() : "";
      break;
    }
  }
  if (clientRowNum < 0) {
    return errorResponse_("Client not found in CB Clients tab. The client spreadsheet may exist in Drive but not be linked to Consolidated Billing — you'll need to manually add the row or re-run onboarding.", "NOT_FOUND");
  }

  // v38.35.0: Discover the bound script ID via multiple fallback strategies:
  // 1. CB Clients "Script ID" column (if already stored from a previous run)
  // 2. Client sheet's Settings tab "_SCRIPT_ID" row (written by verifyTriggers/
  //    StrideClientInstallTriggers — runs whenever the Stride Admin menu fires)
  // 3. Drive search by parent (UNRELIABLE for container-bound scripts — container
  ...
  // This is the MOST RELIABLE source — the client's bound script writes its own
  // ID here whenever StrideClientInstallTriggers() / verifyTriggers() runs
  // (triggered on every Stride Admin menu action). Container-bound scripts have
  // their own hidden parent that Drive queries can't reliably find, but the script
  // can read its own ID via ScriptApp.getScriptId() and store it in the sheet.
  if (!scriptId) {
    try {
      var clientSS = SpreadsheetApp.openById(clientSheetId);
      var clientSettings = clientSS.getSheetByName("Settings");
      if (clientSettings) {
        var lastRow = clientSettings.getLastRow();
        if (lastRow > 0) {
          var settingsData = clientSettings.getRange(1, 1, lastRow, 2).getValues();
          for (var si = 0; si < settingsData.length; si++) {
            ...
            if (key === "_SCRIPT_ID") {
              ...
              if (scriptId) {
                ...
                // Write to CB Clients so next time we don't have to open the client sheet
                if (scriptIdColIdx >= 0) {
      ...
      Logger.log("handleFinishClientSetup_: Could not read client Settings tab: " + settingsErr);
    }
  }

  // Strategy 3: Drive search within the client's FOLDER (most reliable Drive
  // path for container-bound scripts — makeCopy() puts the new bound script in
  // the same folder as the new sheet). Requires us to find the client folder
  // from either the CB Clients row or by querying the sheet's parent folder.
  if (!scriptId) {
    try {
      var folderId = "";
      // Try to read Client Folder ID from the CB Clients row
      var clientFolderIdCol = cbHdr.indexOf("CLIENT FOLDER ID");
      if (clientFolderIdCol >= 0) {
        folderId = String(clientsData[clientRowNum - 1][clientFolderIdCol] || "").trim();
      }
      // Fall back to reading the sheet's parent folder from Drive
      if (!folderId) {
        try {
          var sheetFile = DriveApp.getFileById(clientSheetId);
      ...
      if (folderId) {
        ...
        if (scriptId) {
          Logger.log("handleFinishClientSetup_: Found script ID via folder search: " + scriptId);
          if (scriptIdColIdx >= 0) {
            clientsSh.getRange(clientRowNum, scriptIdColIdx + 1).setValue(scriptId);
          }
        }
      }
    } catch (folderSearchErr) {
      Logger.log("handleFinishClientSetup_: Folder search failed: " + folderSearchErr);
    }
  }
  ...
  Logger.log("handleFinishClientSetup_: Using script ID " + scriptId + " (source: " + discoverySource + ")");

  // v38.36.0: First check if a deployment ALREADY EXISTS for this script.
  // If yes, just read its URL back — no need to create a new version/deployment.
  // This fixes the case where onboarding deployed the Web App successfully but
  // failed to write the URL to CB Clients (e.g. duplicate column issue).
  var newWebAppUrl = "";
  var deploymentId = "";
  ...
  try {
    var token = ScriptApp.getOAuthToken();
    var baseUrl = "https://script.googleapis.com/v1/projects/" + scriptId;

    // Check for existing deployments first
    try {
      var listRes = UrlFetchApp.fetch(baseUrl + "/deployments", {
        method: "get",
        headers: { "Authorization": "Bearer " + token },
        muteHttpExceptions: true
      });
      if (listRes.getResponseCode() === 200) {
        ...
        if (listData.deployments && listData.deployments.length > 0) {
          // Look for an existing Web App deployment (not HEAD deployment)
          for (var di = 0; di < listData.deployments.length; di++) {
            var dep = listData.deployments[di];
            if (dep.entryPoints) {
              for (var dei = 0; dei < dep.entryPoints.length; dei++) {
                if (dep.entryPoints[dei].entryPointType === "WEB_APP") {
                  var depUrl = dep.entryPoints[dei].webApp && dep.entryPoints[dei].webApp.url;
                  // Skip HEAD (dev) deployment — we want a versioned one
                  var isHead = dep.deploymentConfig && dep.deploymentConfig.versionNumber == null;
                  if (depUrl && !isHead) {
                    newWebAppUrl = depUrl;
                    deploymentId = dep.deploymentId;
                    reusedExisting = true;
                    Logger.log("handleFinishClientSetup_: Reusing existing deployment " + deploymentId + " → " + newWebAppUrl);
                    break;
                  }
                }
              }
            }
            if (reusedExisting) break;
          }
        }
      }
    } catch (listErr) {
      Logger.log("handleFinishClientSetup_: Could not list existing deployments: " + listErr);
    }

    // If no existing deployment found, create a new version + deployment
    if (!reusedExisting) {
      // Step 1: Create a version
      var versionRes = UrlFetchApp.fetch(baseUrl + "/versions", {
        method: "post",
        contentType: "application/json",
        headers: { "Authorization": "Bearer " + token },
        payload: JSON.stringify({ description: "Finish Setup — auto-deployed by recovery endpoint" }),
        muteHttpExceptions: true
      });
      var versionData = JSON.parse(versionRes.getContentText());
      var versionNumber = versionData.versionNumber;
      ...
      if (!versionNumber) {
        return errorResponse_(
          "Could not create script version. The Apps Script API may not be enabled, or this script has a different owner. Details: " + versionRes.getContentText().substring(0, 300),
          "DEPLOY_ERROR"
        );
      }
      ...
      var deployRes = UrlFetchApp.fetch(baseUrl + "/deployments", {
        ...
        payload: JSON.stringify({
          versionNumber: versionNumber,
          description: "Client Web App — Finish Setup recovery"
        }),
        muteHttpExceptions: true
      });
      var deployData = JSON.parse(deployRes.getContentText());
    ...
    if (!newWebAppUrl) {
      return errorResponse_(
        "Web App deployment returned no URL. The Apps Script API may have an issue. Try opening Extensions → Apps Script on the client sheet and manually deploying.",
        "DEPLOY_ERROR"
      );
    }

    // v38.36.0: Write to ALL matching columns (handles duplicate "Web App URL" columns)
    try {
      // Build a multi-index map of CB Clients headers
      var cbHeaderAll = {};
      for (var hi = 0; hi < cbHdr.length; hi++) {
        var hkey = cbHdr[hi];
        if (!hkey) continue;
        if (!cbHeaderAll[hkey]) cbHeaderAll[hkey] = [];
        cbHeaderAll[hkey].push(hi);
      }

      var webUrlIndices = cbHeaderAll["WEB APP URL"] || [];
      for (var wi = 0; wi < webUrlIndices.length; wi++) {
        clientsSh.getRange(clientRowNum, webUrlIndices[wi] + 1).setValue(newWebAppUrl);
        Logger.log("handleFinishClientSetup_: Wrote WEB APP URL to row " + clientRowNum + " col " + (webUrlIndices[wi] + 1));
      }

      if (deploymentId) {
        var deployIndices = cbHeaderAll["DEPLOYMENT ID"] || [];
        for (var dpi = 0; dpi < deployIndices.length; dpi++) {
          clientsSh.getRange(clientRowNum, deployIndices[dpi] + 1).setValue(deploymentId);
          Logger.log("handleFinishClientSetup_: Wrote DEPLOYMENT ID to row " + clientRowNum + " col " + (deployIndices[dpi] + 1));
        }
      }

      // Warn about duplicate columns so user knows to clean them up
      var dupCols = [];
      for (var hkCheck in cbHeaderAll) {
        if (cbHeaderAll[hkCheck].length > 1) dupCols.push(hkCheck);
      }
      if (dupCols.length > 0) {
        warnings.push("CB Clients sheet has duplicate columns: " + dupCols.join(", ") + ". Data was written to all copies. Consider deleting the duplicates.");
      }
    } catch (writeErr) {
      warnings.push("Web App deployed but failed to write URL to CB Clients: " + writeErr.message);
    }
    ...
    try {
      ...
      if (triggerCode === 302) {
        var redirectUrl = triggerRes.getHeaders()["Location"] || "";
        if (redirectUrl) {
          UrlFetchApp.fetch(redirectUrl, { muteHttpExceptions: true });
        }
      }
    } catch (trigErr) {
      warnings.push("Trigger install failed: " + trigErr.message + " — you can re-run this later via the Finish Setup button.");
    }
    ...
    return jsonResponse_({
      ...
      clientSheetId: clientSheetId,
      scriptId: scriptId,
      webAppUrl: newWebAppUrl,
      deploymentId: deploymentId,
      warnings: warnings,
      message: "Setup complete — Web App deployed and triggers installed."
    });
...
function handleBulkSyncToSupabase_(payload) {
  ...
  if (targetClient) {
    ...
    // v38.28.0: Column is "Client Spreadsheet ID" not "Spreadsheet ID" (every
    // other handler in this file uses the correct name; this was a pre-existing
    // typo that only surfaced when the per-client loop started calling this path).
    var cbId = prop_("CB_SPREADSHEET_ID");
    var cbSs = SpreadsheetApp.openById(cbId);
    ...
    for (var i = 0; i < clientRows.length; i++) {
      var sid = String(clientRows[i]["Client Spreadsheet ID"] || "").trim();
      if (sid === targetClient) {
  ...
  var totalDeleted = { inventory: 0, tasks: 0, repairs: 0, will_calls: 0, shipments: 0, billing: 0 };

  for (var c = 0; c < clients.length; c++) {
    var client = clients[c];
    var clientResult = { client: client.name, spreadsheetId: client.spreadsheetId, counts: {}, deleted: {}, errors: [] };
    ...
    try {
      ...
      try {
        ...
        if (invSheet) {
          ...
          supabaseBatchUpsert_("inventory", invSb);
          var invDel = supabaseDeleteStaleRows_("inventory", client.spreadsheetId, bkInvKeepIds, "item_id");
          clientResult.counts.inventory = invSb.length;
          clientResult.deleted.inventory = invDel;
          totalRows.inventory += invSb.length;
          totalDeleted.inventory += invDel;
        }
      ...
      try {
        ...
        if (taskSheet) {
          var taskRows = sheetToObjects_(taskSheet);
          var bkTaskFolderUrls = api_readIdFolderUrls_(taskSheet, "Task ID");
          var bkShipFolderMap = api_buildShipmentFolderMap_(ss);
          var taskSb = [];
          var bkTaskKeepIds = [];
          for (var j = 0; j < taskRows.length; j++) {
            var tid = String(taskRows[j]["Task ID"] || "").trim();
            if (!tid) continue;
            bkTaskKeepIds.push(tid);
            var bkShipNo = String(taskRows[j]["Shipment #"] || "").trim();
            taskSb.push(sbTaskRow_(client.spreadsheetId, {
              ...
              location: taskRows[j]["Location"],
              vendor: taskRows[j]["Vendor"] || "", sidemark: taskRows[j]["Sidemark"] || "",
              shipmentNumber: bkShipNo,
              cancelledAt: formatDate_(taskRows[j]["Cancelled At"]),
              startedAt: formatDate_(taskRows[j]["Started At"]),
              billed: String(taskRows[j]["Billed"] || "").toLowerCase() === "true",
              clientName: client.name || "",
              taskFolderUrl: bkTaskFolderUrls[tid] || "",
              shipmentFolderUrl: bkShipFolderMap[bkShipNo] || ""
            }));
          }
          supabaseBatchUpsert_("tasks", taskSb);
          var taskDel = supabaseDeleteStaleRows_("tasks", client.spreadsheetId, bkTaskKeepIds, "task_id");
          clientResult.counts.tasks = taskSb.length;
          clientResult.deleted.tasks = taskDel;
          totalRows.tasks += taskSb.length;
          totalDeleted.tasks += taskDel;
        }
      ...
      try {
        ...
        if (repSheet) {
          var repRows = sheetToObjects_(repSheet);
          var bkRepFolderUrls = api_readIdFolderUrls_(repSheet, "Repair ID");
          ...
          for (var k = 0; k < repRows.length; k++) {
            ...
            repSb.push(sbRepairRow_(client.spreadsheetId, {
              ...
              repairFolderUrl: bkRepFolderUrls[rid] || "",
              shipmentFolderUrl: bkRepShipMap[rShipNo] || "",
      ...
      try {
        ...
        if (wcSheet) {
          ...
          supabaseBatchUpsert_("will_calls", wcSb);
          var wcDel = supabaseDeleteStaleRows_("will_calls", client.spreadsheetId, bkWcKeepIds, "wc_number");
          clientResult.counts.will_calls = wcSb.length;
          clientResult.deleted.will_calls = wcDel;
          totalRows.will_calls += wcSb.length;
          totalDeleted.will_calls += wcDel;
        }
      ...
      try {
        ...
        if (shipSheet) {
          var shipRows = sheetToObjects_(shipSheet);
          var bkShipFolderUrls = api_readIdFolderUrls_(shipSheet, "Shipment #");
          var shipSb = [];
          var bkShipKeepIds = [];
          for (var n = 0; n < shipRows.length; n++) {
            var sn = String(shipRows[n]["Shipment #"] || "").trim();
            if (!sn) continue;
            bkShipKeepIds.push(sn);
            shipSb.push(sbShipmentRow_(client.spreadsheetId, {
              ...
              folderUrl: bkShipFolderUrls[sn] || ""
            }));
          }
          supabaseBatchUpsert_("shipments", shipSb);
          var shipDel = supabaseDeleteStaleRows_("shipments", client.spreadsheetId, bkShipKeepIds, "shipment_number");
          clientResult.counts.shipments = shipSb.length;
          clientResult.deleted.shipments = shipDel;
          totalRows.shipments += shipSb.length;
          totalDeleted.shipments += shipDel;
        }
...
function handleReconcileSupabase_(payload) {
  ...
  var clients;
  if (targetClient) {
    // v38.28.0: Same column-name fix as handleBulkSyncToSupabase_
    var cbId = prop_("CB_SPREADSHEET_ID");
    var cbSs = SpreadsheetApp.openById(cbId);
    var clientsSheet = cbSs.getSheetByName("Clients");
    var clientRows = sheetToObjects_(clientsSheet);
...
// ═══════════════════════════════════════════════════════════════════════════════
// QBO (QuickBooks Online) Integration — v38.19.0
// OAuth 2.0 Authorization Code flow + Customer/Sub-Job Auto-Create + Invoice Push
// ═══════════════════════════════════════════════════════════════════════════════

var QBO_RETRYABLE_STATUS_CODES_ = [429, 500, 502, 503, 504];

// ─── QBO OAuth Helpers ──────────────────────────────────────────────────────

/**
 * Escape single quotes for QBO query string interpolation.
 * All QBO queries MUST use this on user-supplied values.
 */
function qbo_escapeQueryString_(str) {
  return String(str || "").replace(/'/g, "\\'");
}

/**
 * Generate QBO authorization URL. Stores nonce for CSRF validation.
 */
function qbo_getAuthUrl_() {
  var clientId    = prop_("QBO_CLIENT_ID");
  var redirectUri = prop_("QBO_REDIRECT_URI");
  if (!clientId)    throw new Error("QBO_CLIENT_ID not configured in Script Properties");
  if (!redirectUri) throw new Error("QBO_REDIRECT_URI not configured in Script Properties");
  ...
  PropertiesService.getScriptProperties().setProperty("QBO_STATE_NONCE", nonce);
 ...
 * Validates state nonce to prevent CSRF. Stores tokens in PropertiesService.
 */
function qbo_exchangeCode_(code, state, realmId) {
  if (!code)  return { success: false, error: "Missing authorization code" };
  if (!state) return { success: false, error: "Missing state parameter" };

  var storedNonce = prop_("QBO_STATE_NONCE");
  if (!storedNonce || state !== storedNonce) {
    return { success: false, error: "Invalid state — possible CSRF. Please try connecting again." };
  }

  var clientId     = prop_("QBO_CLIENT_ID");
  var clientSecret = prop_("QBO_CLIENT_SECRET");
  var redirectUri  = prop_("QBO_REDIRECT_URI");

  if (!clientId || !clientSecret) {
    return { success: false, error: "QBO_CLIENT_ID or QBO_CLIENT_SECRET not configured" };
  }

  try {
    var response = UrlFetchApp.fetch("https://oauth.platform.intuit.com/oauth2/v1/tokens/bearer", {
      method: "post",
      headers: {
    ...
    if (status !== 200 || !body.access_token) {
      return { success: false, error: "Token exchange failed: " + (body.error_description || body.error || "HTTP " + status) };
    }

    var props = PropertiesService.getScriptProperties();
    props.setProperty("QBO_ACCESS_TOKEN",  body.access_token);
    props.setProperty("QBO_REFRESH_TOKEN", body.refresh_token || "");
    if (realmId) props.setProperty("QBO_REALM_ID", realmId);
    props.deleteProperty("QBO_STATE_NONCE");

    return { success: true };
  } catch (e) {
    return { success: false, error: "Token exchange error: " + e.message };
  }
}
 ...
 */
function qbo_refreshToken_() {
  var refreshToken = prop_("QBO_REFRESH_TOKEN");
  if (!refreshToken) throw new Error("QBO not connected — re-authorize required");
  ...
  if (status !== 200 || !body.access_token) {
    // Refresh token likely expired (100-day lifetime) — must re-authorize
    throw new Error("QBO token refresh failed — re-authorize required. " + (body.error_description || body.error || "HTTP " + status));
  }

  var props = PropertiesService.getScriptProperties();
  props.setProperty("QBO_ACCESS_TOKEN", body.access_token);
  if (body.refresh_token) props.setProperty("QBO_REFRESH_TOKEN", body.refresh_token);
...
function qbo_apiRequest_(method, path, payload, token, realmId) {
  ...
  for (var attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (status === 401 && attempt === 0) {
        currentToken = qbo_refreshToken_();
        continue; // retry immediately (no backoff for auth refresh)
      }

      // Non-retryable error
      if (QBO_RETRYABLE_STATUS_CODES_.indexOf(status) === -1) {
        var errMsg = "HTTP " + status;
        if (data && data.Fault && data.Fault.Error && data.Fault.Error.length > 0) {
          errMsg += ": " + data.Fault.Error[0].Message + (data.Fault.Error[0].Detail ? " — " + data.Fault.Error[0].Detail : "");
        } else if (bodyText) {
          errMsg += ": " + bodyText.substring(0, 300);
        }
        return { success: false, status: status, data: data, error: errMsg };
      }
  ...
  return { success: false, status: 0, data: null, error: "QBO API: max retries exceeded" };
}

/**
 * Get a valid QBO access token, refreshing if needed.
 */
function qbo_getValidToken_() {
  ...
  // Lightweight validation: try to read CompanyInfo
  var result = qbo_apiRequest_("GET", "companyinfo/" + realmId, null, token, realmId);
  if (result.success) return token;

  // If we get here with a 401, the retry inside qbo_apiRequest_ already refreshed
  // Check if the refresh succeeded by reading the new token
  var refreshedToken = prop_("QBO_ACCESS_TOKEN");
  if (refreshedToken && refreshedToken !== token) return refreshedToken;

  throw new Error("QBO token validation failed — please re-authorize");
}
