...
- **StrideAPI.gs:** v38.61.0 (Web App v281) — session 70 nine-item UI/email/PDF fix batch. NEW `handleGetPaymentTerms_` endpoint reads the CB `Payment_Terms` tab (auto-seeded with the legacy 6 options on first call). FIX `sbShipmentRow_` now strips the `[IK:<uuid>]` idempotency-key prefix before Supabase write-through (was leaking into the React Shipment Notes field). FIX `handleSendRepairQuote_` resolves the VIEW INSPECTION PHOTOS URL from Inventory Item ID hyperlink → Repairs Source Task hyperlink → client `PHOTOS_FOLDER_ID` instead of the hardcoded `"#"` placeholder. Also carries v38.60.x: Repair Work Order PDF full-token expansion (all 6 previously-literal `{{TOKEN}}` fields render) and `handleGetBatch_` client-role field parity.
