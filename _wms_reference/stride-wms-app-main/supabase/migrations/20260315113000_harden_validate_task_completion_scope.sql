-- Harden validate_task_completion tenant isolation and execute permissions
...
BEGIN
  ...
  IF (v_item_ids IS NULL OR array_length(v_item_ids, 1) IS NULL) AND v_task.related_item_id IS NOT NULL THEN
    PERFORM 1
    FROM public.items i
    WHERE i.id = v_task.related_item_id
      AND i.tenant_id = v_tenant_id
      AND i.deleted_at IS NULL;
  ...
  ELSIF v_task.task_type = 'Repair' THEN
    SELECT NOT EXISTS (
      ...
      WHERE rq.tenant_id = v_tenant_id
        AND (rq.source_task_id = p_task_id OR rq.item_id = ANY(v_item_ids))
...
REVOKE ALL ON FUNCTION public.validate_task_completion(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.validate_task_completion(uuid) FROM anon;
REVOKE ALL ON FUNCTION public.validate_task_completion(uuid) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.validate_task_completion(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.validate_task_completion(uuid) TO service_role;
