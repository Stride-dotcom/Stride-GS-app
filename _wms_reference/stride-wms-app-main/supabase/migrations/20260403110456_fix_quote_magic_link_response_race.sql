-- Prevent concurrent public quote responses from applying conflicting final states.
-- Lock the quote row by magic link token so only one response transaction can proceed at a time.
