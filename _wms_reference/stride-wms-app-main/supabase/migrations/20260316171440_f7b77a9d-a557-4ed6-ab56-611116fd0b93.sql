...
BEGIN
  ...
  FOR i IN 0..(p_leftover_qty - 1) LOOP
    ...
    INSERT INTO public.items (
      id, tenant_id, account_id, warehouse_id, item_code, description,
      item_type_id, current_location_id,
      status, condition, quantity,
      size, size_unit,
      sidemark_id, sidemark, room, class_id,
      client_account, vendor, sku, category,
      receiving_shipment_id,
      weight_lbs, declared_value,
      coverage_type, coverage_deductible, coverage_rate, coverage_source,
      created_at, updated_at,
      metadata
    )
    VALUES (
      v_new_id, v_tenant_id, v_parent.account_id, v_parent.warehouse_id,
      v_parent.item_code || '-' || (v_start_suffix + i)::text,
      v_parent.description,
      v_parent.item_type_id, v_target_location,
      v_parent.status, v_parent.condition, 1,
      v_parent.size, v_parent.size_unit,
      v_parent.sidemark_id, v_parent.sidemark, v_parent.room, v_parent.class_id,
      v_parent.client_account, v_parent.vendor, v_parent.sku, v_parent.category,
      v_parent.receiving_shipment_id,
      v_parent.weight_lbs, v_parent.declared_value,
      v_parent.coverage_type, v_parent.coverage_deductible, v_parent.coverage_rate, v_parent.coverage_source,
      now(), now(),
    ...
    INSERT INTO public.movements (
      id, tenant_id, item_id, action_type, actor_type,
      from_location_id, to_location_id,
      actor_id, note, created_at
    )
    VALUES (
      gen_random_uuid(), v_tenant_id, v_new_id, 'move', 'system',
  ...
  VALUES (
    gen_random_uuid(), v_tenant_id, p_parent_item_id, 'move', 'system',
