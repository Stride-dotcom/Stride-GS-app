-- =============================================================================
-- BUG-BETA-01 v2: Fix rpc_find_inbound_candidates — add carrier as qualifying
-- cross-account signal and ensure function + GRANT are applied idempotently.
-- =============================================================================

-- Drop ALL known overloads to prevent ambiguity
DROP FUNCTION IF EXISTS public.rpc_find_inbound_candidates(UUID, TEXT, TEXT, INTEGER);
DROP FUNCTION IF EXISTS public.rpc_find_inbound_candidates(UUID, TEXT, TEXT, INTEGER, TEXT, TEXT);
  ...
  FROM (
    ...
    FROM (
      ...
      WHERE s.tenant_id = v_tenant_id
        ...
        AND (s.inbound_status IS NULL OR s.inbound_status IN ('draft', 'stage1_complete', 'receiving'))
        AND s.created_at >= now() - interval '90 days'
        AND s.deleted_at IS NULL
        -- Candidate inclusion filter: same account always included.
        -- Cross-account (or no-account) requires a signal match
        -- (tracking, reference, carrier, or vendor name).
        AND (
          ...
          OR
          (v_norm_carrier IS NOT NULL AND s.carrier IS NOT NULL
           AND LOWER(s.carrier) ILIKE '%' || v_norm_carrier || '%')
          OR
          (v_norm_vendor IS NOT NULL AND s.vendor_name IS NOT NULL
           AND LOWER(s.vendor_name) ILIKE '%' || v_norm_vendor || '%')
        )
    ) sub
